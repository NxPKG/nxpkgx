...
import { NXPKG_BIN, type TTFTData } from "../helpers";
import { run } from "./run";
...
if (!fs.existsSync(NXPKG_BIN)) {
  throw new Error("No nxpkg binary found");
}
...
for (const item of profileJSON) {
  ...
  if (args.nxpkg_version) {
    ttftData.nxpkgVersion = `${args.nxpkg_version}`;
  }
