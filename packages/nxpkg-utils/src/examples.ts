...
export async function getRepoInfo(
  ...
  if (
    // Support repos whose entire purpose is to be a Nxpkgrepo example, e.g.
    // https://github.com/:username/:my-cool-nxpkgrepo-example-repo-name.
    tree === undefined ||
    // Support GitHub URL that ends with a trailing slash, e.g.
    // https://github.com/:username/:my-cool-nxpkgrepo-example-repo-name/
    // In this case "t" will be an empty string while the nxpkg part "_branch" will be undefined
...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/vercel/nxpkg/contents/examples/${encodeURIComponent(
        nameOrUrl
      )}`
    );
  }
...
export async function downloadAndExtractExample(root: string, name: string) {
  const tempFile = await downloadTar(
    `https://codeload.github.com/nxpkg/nxpkg/tar.gz/main`,
    `nxpkg-ct-example`
  );
  ...
  await extract({
    ...
    filter: (p: string) => p.includes(`nxpkg-main/examples/${name}/`),
  });
