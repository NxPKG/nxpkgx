...
import { nxpkgIgnore } from "../src/ignore";
...
describe("nxpkgIgnore()", () => {
  ...
  it("throws error and allows build when exec fails", () => {
    ...
    nxpkgIgnore("test-workspace", {});

    expect(mockExec).toHaveBeenCalledWith(
      `npx nxpkg run build --filter="test-workspace...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
  ...
  it("throws pretty error and allows build when exec fails", () => {
    ...
    validateLogs(
      [
        `nxpkg-ignore could not complete - no package manager detected, please commit a lockfile, or set "packageManager" in your root "package.json"`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("throws pretty error and allows build when can't find previous sha", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx nxpkg run build --filter="test-workspace...[too-far-back]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    ...
    validateLogs(
      [
        `nxpkg-ignore could not complete - a ref or SHA is invalid. It could have been removed from the branch history via a force push, or this could be a shallow clone with insufficient history`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("throws pretty error and allows build when fallback fails", () => {
    ...
    nxpkgIgnore("test-workspace", { fallback: "HEAD^" });
    ...
    validateLogs(
      [
        `nxpkg-ignore could not complete - parent commit does not exist or is unreachable`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("skips checks and allows build when no workspace can be found", () => {
    nxpkgIgnore(undefined, { directory: "__fixtures__/no-app" });
    validateLogs(
      [
        () => [
          "≫  ",
          expect.stringContaining(
            " could not be found. nxpkg-ignore inferencing failed"
          ),
        ],
      ],
      mockConsole.error,
  ...
  it("skips checks and allows build when a workspace with no name is found", () => {
    nxpkgIgnore(undefined, { directory: "__fixtures__/invalid-app" });
    validateLogs(
      [
        () => [
          "≫  ",
  ...
  it("skips checks and allows build when no monorepo root can be found", () => {
    nxpkgIgnore(undefined, { directory: "/" });
    expectBuild(mockExit);
    expect(mockConsole.error).toHaveBeenLastCalledWith(
      "≫  ",
      "Monorepo root not found. nxpkg-ignore inferencing failed"
    );
  });

  it("skips checks and allows build when NXPKG_FORCE is set", () => {
    process.env.NXPKG_FORCE = "true";
    nxpkgIgnore(undefined, { directory: "test-workspace" });
    expect(mockConsole.log).toHaveBeenNthCalledWith(
      2,
      "≫  ",
      "`NXPKG_FORCE` detected"
    );
    expectBuild(mockExit);
  });
  ...
  it("allows build when no comparison is returned", () => {
    ...
    process.env.VERCEL_GIT_COMMIT_REF = "my-branch";
    nxpkgIgnore("test-app", { directory: "__fixtures__/app" });
    expect(mockConsole.log).toHaveBeenNthCalledWith(
      4,
      "≫  ",
  ...
  it("skips build for `previousDeploy` comparison with no changes", () => {
    ...
    nxpkgIgnore(undefined, { directory: "__fixtures__/app" });
    validateLogs(
      [
        "Using Nxpkgrepo to determine if this project is affected by the commit...\n",
        ...
        'Analyzing results of `npx nxpkg run build --filter="test-app...[last-deployed-sha]" --dry=json`',
  ...
  it("allows build for `previousDeploy` comparison with changes", () => {
    ...
    validateLogs(
      [
        ...
        'Found previous deployment ("last-deployed-sha") for "test-app" on branch "my-branch"',
        'Analyzing results of `npx nxpkg run "workspace#build" --filter="test-app...[last-deployed-sha]" --dry=json`',
  ...
  it("allows build for unavailable `previousDeploy` comparison with fallback", () => {
    ...
    validateLogs(
      [
        ...
        'Analyzing results of `npx nxpkg run "workspace#build" --filter="test-app...[HEAD^2]" --dry=json`',
        'This commit affects "test-app"',
        () => expect.stringContaining("✓ Proceeding with deployment"),
      ],
      mockConsole.log,
  ...
  it("throws error and allows build when json cannot be parsed", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx nxpkg run build --filter="test-app...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    validateLogs(
      [
        'Failed to parse JSON output from `npx nxpkg run build --filter="test-app...[HEAD^]" --dry=json`.',
      ],
      mockConsole.error,
      { prefix: "≫  " }
    );
  ...
  it("runs full nxpkg-ignore check when commit message contains a conflicting string", () => {
    process.env.VERCEL = "1";
  ...
  it("passes max buffer to nxpkg exectuion", () => {
    const mockExec = jest
      .spyOn(child_process, "exec")
      .mockImplementation((command, options, callback) => {
        if (callback) {
    ...
    nxpkgIgnore(undefined, { directory: "__fixtures__/app", maxBuffer: 1024 });
