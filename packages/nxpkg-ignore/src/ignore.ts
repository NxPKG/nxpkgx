...
import { getNxpkgRoot } from "@turbo/utils";
...
import type { NxpkgIgnoreArg, NxpkgIgnoreOptions } from "./types";
...
export function nxpkgIgnore(
  workspaceArg: NxpkgIgnoreArg,
  opts: NxpkgIgnoreOptions
) {
  const inputs = {
  ...
  // check for NXPKG_FORCE and bail early if it's set
  if (process.env.NXPKG_FORCE === "true") {
    info("`NXPKG_FORCE` detected");
    return continueBuild();
  }

  // find the monorepo root
  const root = getNxpkgRoot(inputs.directory);
  if (!root) {
    error("Monorepo root not found. nxpkg-ignore inferencing failed");
    return continueBuild();
  }
  ...
  const command = `npx nxpkg run ${task} --filter="${workspace}...[${comparison.ref}]" --dry=json`;
