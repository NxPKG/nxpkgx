...
const { nxpkgGradient, nxpkgLoader, info, error, warn } = logger;
...
export async function create(
  ...
  logger.log(chalk.bold(nxpkgGradient(`\n>>> NXPKGREPO\n`)));
  info(`Welcome to Nxpkgrepo! Let's get you set up with a new codebase.`);
  logger.log();
  ...
  tryGitInit(root, `feat(create-nxpkg): create ${exampleName}`);
    ...
    for (const transform of transforms) {
      try {
        ...
        if (transformResult.result === "success") {
          ...
          tryGitAdd();
          tryGitCommit(
            `feat(create-nxpkg): apply ${transformResult.name} transform`
          );
        }
      } catch (err) {
        handleErrors(err);
  ...
  info("Created a new Nxpkgrepo with the following:");
  logger.log();
  ...
  if (hasPackageJson && !skipInstall) {
    ...
    if (
      ...
      const loader = nxpkgLoader("Installing dependencies...").start();
      await install({
        project,
      ...
      tryGitCommit("feat(create-nxpkg): install dependencies");
      loader.stop();
    }
  }
  ...
  if (projectDirIsCurrentDir) {
    logger.log(
      `${chalk.bold(
        nxpkgGradient(">>> Success!")
      )} Your new Nxpkgrepo is ready.`
    );
  } else {
    logger.log(
      `${chalk.bold(
        nxpkgGradient(">>> Success!")
      )} Created a new Nxpkgrepo at "${relativeProjectDir}".`
    );
  }
  ...
  if (packageManagerMeta && hasPackageJson) {
    ...
    logger.log(`Nxpkgrepo will cache locally by default. For an additional`);
    logger.log(`speed boost, enable Remote Caching with Vercel by`);
    logger.log(`entering the following command:`);
    logger.log();
    logger.log(chalk.cyan(`  ${packageManagerMeta.executable} nxpkg login`));
    logger.log();
