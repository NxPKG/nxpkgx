...
import { getNxpkgUpgradeCommand } from "./steps/getNxpkgUpgradeCommand";
...
Migration is done in 4 steps:
  -- gather information
  1. find the version (x) of nxpkg to migrate from (if not specified)
  2. find the version (y) of nxpkg to migrate to (if not specified)
  ...
  5. update the nxpkg version (optionally)
**/
export async function migrate(
  ...
  if (!fromVersion) {
    return endMigration({
      success: false,
      message: `Unable to infer the version of nxpkg being used by ${project.name}`,
    });
  }
  ...
  if (!toVersion) {
    return endMigration({
      success: false,
      message: `Unable to fetch the latest version of nxpkg`,
    });
  }
  ...
  // shutdown the nxpkg daemon before running codemods and upgrading
  ...
  // step 4
  logger.log(
    `Upgrading nxpkg from ${chalk.bold(fromVersion)} to ${chalk.bold(
      toVersion
    )} (${
      codemods.length === 0
  ...
  if (options.install) {
    if (options.dry) {
      logger.log(
        `Upgrading nxpkg with ${chalk.bold(upgradeCommand)} ${chalk.dim(
          "(dry run)"
        )}`,
        os.EOL
      );
    } else {
      logger.log(`Upgrading nxpkg with ${chalk.bold(upgradeCommand)}`, os.EOL);
      try {
        execSync(upgradeCommand, { stdio: "pipe", cwd: project.paths.root });
      } catch (err: unknown) {
        return endMigration({
          success: false,
          message: `Unable to upgrade nxpkg: ${String(err)}`,
        });
      }
    }
  } else {
    logger.log(`Upgrade nxpkg with ${chalk.bold(upgradeCommand)}`, os.EOL);
  }
