...
import * as getNxpkgUpgradeCommand from "../src/commands/migrate/steps/getNxpkgUpgradeCommand";
...
describe("migrate", () => {
  ...
  it("migrates from 1.0.0 to 1.7.0", async () => {
    ...
    const mockedGetNxpkgUpgradeCommand = jest
      .spyOn(getNxpkgUpgradeCommand, "getNxpkgUpgradeCommand")
      .mockResolvedValue("pnpm install -g nxpkg@latest");
    ...
    expect(mockedGetNxpkgUpgradeCommand).toHaveBeenCalled();
    ...
    mockedGetNxpkgUpgradeCommand.mockRestore();
  ...
  it("migrates from 1.0.0 to 1.2.0 (dry run)", async () => {
    ...
    const nxpkgJson = readJson("nxpkg.json");
    ...
    expect(readJson("nxpkg.json")).toStrictEqual(nxpkgJson);
  ...
  it("continues when migration doesn't require codemods", async () => {
    ...
    const mockedGetNxpkgUpgradeCommand = jest
      .spyOn(getNxpkgUpgradeCommand, "getNxpkgUpgradeCommand")
      .mockResolvedValue("npm install nxpkg@1.3.1");
    ...
    expect(mockedGetNxpkgUpgradeCommand).toHaveBeenCalled();
    expect(mockedGetWorkspaceDetails).toHaveBeenCalled();
    expect(mockedExecSync).toHaveBeenNthCalledWith(1, "nxpkg bin", {
      cwd: root,
      stdio: "ignore",
    });
    expect(mockedExecSync).toHaveBeenNthCalledWith(2, "nxpkg daemon stop", {
      cwd: root,
      stdio: "ignore",
    });
    expect(mockedExecSync).toHaveBeenNthCalledWith(
      3,
      "npm install nxpkg@1.3.1",
      {
        cwd: root,
        stdio: "pipe",
      }
  ...
  it("installs the correct nxpkg version", async () => {
    const { root, readJson } = useFixture({
      fixture: "old-nxpkg",
    });
    ...
    const mockedGetNxpkgUpgradeCommand = jest
      .spyOn(getNxpkgUpgradeCommand, "getNxpkgUpgradeCommand")
      .mockResolvedValue("pnpm install -g nxpkg@1.7.0");
    const mockedGetAvailablePackageManagers = jest
      .spyOn(nxpkgUtils, "getAvailablePackageManagers")
      .mockResolvedValue({
        pnpm: packageManagerVersion,
