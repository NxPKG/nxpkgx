...
export class NodeModuleTracePlugin implements WebpackPluginInstance {
  ...
  private async runTrace() {
    ...
    let nxpkgTracingPackagePath = "";
    let nxpkgTracingBinPath = "";
    try {
      nxpkgTracingPackagePath = require.resolve(
        "@vercel/experimental-nft/package.json"
      );
    } catch (e) {
      // eslint-disable-next-line no-console -- log feedback
      console.warn(
        `Could not resolve the @vercel/experimental-nft directory, nxpkg tracing may fail.`
      );
    }
    if (nxpkgTracingPackagePath) {
      try {
        const nxpkgTracingBinPackageJsonPath = require.resolve(
          `@vercel/experimental-nft-${process.platform}-${process.arch}/package.json`,
          {
            paths: [join(nxpkgTracingPackagePath, "..")],
          }
        );
        nxpkgTracingBinPath = join(nxpkgTracingBinPackageJsonPath, "..");
      } catch (e) {
        // eslint-disable-next-line no-console -- log feedback
        console.warn(
          `Could not resolve the @vercel/experimental-nft-${process.platform}-${process.arch} directory, nxpkg tracing may fail.`
        );
      }
    }
    const pathSep = process.platform === "win32" ? ";" : ":";
    let paths = `${this.options?.path ?? ""}${pathSep}${process.env.PATH}`;
    if (nxpkgTracingBinPath) {
      paths = `${nxpkgTracingBinPath}${pathSep}${paths}`;
    }
...
function traceChunks(
  ...
  const nxpkgTracingProcess = spawn("node-file-trace", [...args, ...chunks], {
    stdio: "pipe",
    env: {
      ...process.env,
      PATH: paths,
  ...
  return new Promise<void>((resolve, reject) => {
    nxpkgTracingProcess.on("error", (err) => {
      // eslint-disable-next-line no-console -- log error
      console.error(err);
    });
    nxpkgTracingProcess.stdout.on("data", (chunk: string | Uint8Array) => {
      process.stdout.write(chunk);
    });
    nxpkgTracingProcess.stderr.on("data", (chunk: string | Uint8Array) => {
      process.stderr.write(chunk);
    });
    nxpkgTracingProcess.once("exit", (code) => {
      if (!code) {
        resolve();
      } else {
        reject(code);
