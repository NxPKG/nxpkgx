...
export async function generate({
  ...
  if (!generators.length) {
    ...
    if (answer) {
      ...
      logger.log();
      logger.dimmed(
        "Learn more about custom Nxpkgrepo generators - https://nxpkg.build/repo/docs/core-concepts/monorepos/code-generation#custom-generators"
      );
      return;
    }
  }
  ...
  try {
    ...
    if (isOnboarding) {
      logger.log();
      logger.info(`Congrats! You just ran your first Nxpkgrepo generator`);
      logger.dimmed(
        "Learn more about custom Nxpkgrepo generators - https://nxpkg.build/repo/docs/core-concepts/monorepos/code-generation#custom-generators"
      );
    }
  ...
  logger.log();
  logger.bold(logger.nxpkgGradient(">>> Success!"));
}
