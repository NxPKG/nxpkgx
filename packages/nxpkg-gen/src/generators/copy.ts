...
import type { NxpkgGeneratorArguments } from "./types";

export async function generate({ project, opts }: NxpkgGeneratorArguments) {
  ...
  if (opts.copy.type === "external") {
    ...
    const loader = logger.nxpkgLoader(
  ...
  logger.log();
  logger.log(
    `${chalk.bold(logger.nxpkgGradient(">>> Success!"))} Created ${name} at "${
      location.relative
    }"`
  );
}
