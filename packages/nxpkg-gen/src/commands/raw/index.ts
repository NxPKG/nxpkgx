...
import { workspace, type NxpkgGeneratorCLIOptions } from "../workspace";
 ...
 *
 * Used by the nxpkg Rust cli to handoff commands to the \@turbo/gen binary
 */
export async function raw(command: string, options: { json: string }) {
  if (command === "workspace") {
    ...
    await workspace(parsedArgs as NxpkgGeneratorCLIOptions);
  } else if (command === "run") {
