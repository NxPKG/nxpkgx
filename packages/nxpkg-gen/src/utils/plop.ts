...
import { searchUp, getNxpkgConfigs, logger } from "@turbo/utils";
import { GeneratorError } from "./error";

const SUPPORTED_CONFIG_EXTENSIONS = ["ts", "js", "cjs"];
const NXPKG_GENERATOR_DIRECTORY = path.join("nxpkg", "generators");
...
const SUPPORTED_WORKSPACE_GENERATOR_CONFIGS = SUPPORTED_CONFIG_EXTENSIONS.map(
  (ext) => path.join(NXPKG_GENERATOR_DIRECTORY, `config.${ext}`)
);
...
function injectNxpkgrepoData({
  ...
  let nxpkgConfigs = {};
  try {
    nxpkgConfigs = getNxpkgConfigs(generator.basePath);
  } catch (e) {
    // ignore
  }
...
export async function runCustomGenerator({
  ...
  const results = await gen.runActions(
    { ...answers, ...injectNxpkgrepoData({ project, generator: gen }) },
    {
