...
const meta: Rule.RuleMetaData = {
  ...
  docs: {
    description:
      "Do not allow the use of `process.env` without including the env key in any nxpkg.json",
    category: "Configuration Issues",
    recommended: true,
    url: `https://github.com/nxpkg/nxpkg/tree/main/packages/eslint-plugin-nxpkg/docs/rules/${RULES.noUndeclaredEnvVars}.md`,
  },
  schema: [
    {
      type: "object",
...
function create(context: RuleContextWithOptions): Rule.RuleListener {
  ...
  const hasWorkspaceConfigs = project.projectWorkspaces.some(
    (workspaceConfig) => Boolean(workspaceConfig.nxpkgConfig)
  );
  ...
  const checkKey = (node: Node, envKey?: string) => {
    ...
    let message = `{{ envKey }} is not listed as a dependency in ${
      hasWorkspaceConfigs ? "root nxpkg.json" : "nxpkg.json"
    }`;
    if (workspaceConfig?.nxpkgConfig) {
      if (cwd) {
        // if we have a cwd, we can provide a relative path to the workspace config
        message = `{{ envKey }} is not listed as a dependency in the root nxpkg.json or workspace (${path.relative(
          cwd,
          workspaceConfig.workspacePath
        )}) nxpkg.json`;
      } else {
        message = `{{ envKey }} is not listed as a dependency in the root nxpkg.json or workspace nxpkg.json`;
      }
    }
