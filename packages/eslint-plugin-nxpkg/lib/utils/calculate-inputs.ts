...
function processGlobal(
  workspacePath: string,
  rootNxpkgJson: RootSchema
): EnvironmentConfig {
  return {
    legacyConfig: processLegacyConfig(rootNxpkgJson.globalDependencies),
    env: processEnv(rootNxpkgJson.globalEnv),
    passThroughEnv: processPassThroughEnv(rootNxpkgJson.globalPassThroughEnv),
    dotEnv: processDotEnv(workspacePath, rootNxpkgJson.globalDotEnv),
  };
}
...
function getTaskAddress(taskName: string): {
  ...
  if (taskName.length === 0) {
    throw new Error("Invalid task name found in nxpkg.json.");
  }
...
export class Project {
  ...
  generateKey(): ProjectKey {
    ...
    if (this.projectRoot?.nxpkgConfig && !("extends" in this.projectRoot)) {
      const rootNxpkgJson = this.projectRoot;

      global = processGlobal(
        this.projectRoot.workspacePath,
        this.projectRoot.nxpkgConfig
      );

      Object.entries(this.projectRoot.nxpkgConfig.pipeline).forEach(
    ...
    this.projectWorkspaces.forEach((projectWorkspace) => {
      if (!projectWorkspace.nxpkgConfig) {
        return;
      }

      Object.entries(projectWorkspace.nxpkgConfig.pipeline).forEach(
        ([taskName, taskDefinition]) => {
          const { workspaceName: erroneousWorkspaceName, scriptName } =
            getTaskAddress(taskName);
          if (erroneousWorkspaceName) {
            throw new Error(
              "May not specify workspace name in non-root nxpkg.json"
            );
          }
