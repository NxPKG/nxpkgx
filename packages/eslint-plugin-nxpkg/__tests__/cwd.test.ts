...
describe("eslint cache is busted", () => {
  ...
  it("catches a lint error after changing config", () => {
      ...
      expect(outputJson).toMatchObject([
        {
          messages: [
            {
              message:
                "NONEXISTENT is not listed as a dependency in nxpkg.json",
            },
          ],
        },
      ]);
    ...
    const nxpkgJson = readJson<Schema>("nxpkg.json");
    if (nxpkgJson && "globalEnv" in nxpkgJson) {
      nxpkgJson.globalEnv = ["CI", "NONEXISTENT"];
      write("nxpkg.json", JSON5.stringify(nxpkgJson, null, 2));
    }
