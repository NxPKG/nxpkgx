...
template.name = `nxpkg-${os}-${humanizedArchLookup[arch]}`;
template.description = `The ${os}-${humanizedArchLookup[arch]} binary for nxpkg, a monorepo build system.`;
...
if (os === "windows") {
  fs.copyFileSync(
    path.join(__dirname, "template", "bin", "nxpkg"),
    path.join(outputPath, "bin", "nxpkg")
  );
}
fs.copyFileSync(
...
const goBin = os === "windows" ? "go-nxpkg.exe" : "go-nxpkg";
fs.copyFileSync(
