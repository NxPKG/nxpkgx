...
use nxpkg_tasks::{
    ...
    StatsType, TransientInstance, NxpkgTasks, NxpkgTasksBackendApi, UpdateInfo, Value, Vc,
};
use nxpkg_tasks_fs::FileSystem;
use nxpkg_tasks_malloc::NxpkgMalloc;
use nxpkg_tasks_memory::MemoryBackend;
use nxpkgpack::evaluate_context::node_build_environment;
use nxpkgpack_cli_utils::issue::{ConsoleUi, LogOptions};
use nxpkgpack_core::{
...
use nxpkgpack_dev::DevChunkingContext;
use nxpkgpack_dev_server::{
...
use nxpkgpack_env::dotenv::load_env;
use nxpkgpack_node::execution_context::ExecutionContext;
...
pub(crate) mod nxpkg_tasks_viz;
pub(crate) mod web_entry_source;

pub struct NxpkgpackDevServerBuilder {
...
impl NxpkgpackDevServerBuilder {
    pub fn new(
        nxpkg_tasks: Arc<NxpkgTasks<MemoryBackend>>,
        project_dir: String,
        root_dir: String,
    ) -> NxpkgpackDevServerBuilder {
        NxpkgpackDevServerBuilder {
            nxpkg_tasks,
            project_dir,
            root_dir,
    ...
    pub fn entry_request(mut self, entry_asset_path: EntryRequest) -> NxpkgpackDevServerBuilder {
        self.entry_requests.push(entry_asset_path);
        self
    }

    pub fn eager_compile(mut self, eager_compile: bool) -> NxpkgpackDevServerBuilder {
        self.eager_compile = eager_compile;
        self
    }

    pub fn hostname(mut self, hostname: IpAddr) -> NxpkgpackDevServerBuilder {
        self.hostname = Some(hostname);
        self
    }

    pub fn port(mut self, port: u16) -> NxpkgpackDevServerBuilder {
        self.port = Some(port);
        self
    }

    pub fn browserslist_query(mut self, browserslist_query: String) -> NxpkgpackDevServerBuilder {
        self.browserslist_query = browserslist_query;
        self
    }

    pub fn log_level(mut self, log_level: IssueSeverity) -> NxpkgpackDevServerBuilder {
        self.log_level = log_level;
        self
    }

    pub fn show_all(mut self, show_all: bool) -> NxpkgpackDevServerBuilder {
        self.show_all = show_all;
        self
    }

    pub fn allow_retry(mut self, allow_retry: bool) -> NxpkgpackDevServerBuilder {
        self.allow_retry = allow_retry;
        self
    }

    pub fn log_detail(mut self, log_detail: bool) -> NxpkgpackDevServerBuilder {
    ...
    pub async fn build(self) -> Result<DevServer> {
        ...
        let nxpkg_tasks = self.nxpkg_tasks;
...
async fn source(
    ...
    eager_compile: bool,
    nxpkg_tasks: TransientInstance<NxpkgTasks<MemoryBackend>>,
    browserslist_query: String,
    ...
    let project_path: Vc<nxpkg_tasks_fs::FileSystemPath> = fs.root().join(project_relative);

    let env = load_env(project_path);
    let build_output_root = output_fs.root().join(".nxpkgpack/build".to_string());
    ...
    let viz = Vc::upcast(nxpkg_tasks_viz::NxpkgTasksSource::new(nxpkg_tasks.into()));
    ...
    let source = Vc::upcast(PrefixedRouterContentSource::new(
        Default::default(),
        vec![
            ("__nxpkgpack__".to_string(), introspect),
            ("__nxpkg_tasks__".to_string(), viz),
        ],
        main_source,
    ));
...
pub async fn start_server(args: &DevArguments) -> Result<()> {
    ...
    let tt = NxpkgTasks::new(MemoryBackend::new(
        args.common
            .memory_limit
    ...
    let mut server = NxpkgpackDevServerBuilder::new(tt, project_dir, root_dir)
    ...
    let stats_future = async move {
        if args.common.log_detail {
            println!(
                ...
                memory = FormatBytes(NxpkgMalloc::memory_usage())
            );
        }
...
async fn profile_timeout<T>(tt: &NxpkgTasks<MemoryBackend>, future: impl Future<Output = T>) -> T {
