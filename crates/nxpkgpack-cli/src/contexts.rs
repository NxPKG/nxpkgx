...
use nxpkg_tasks_fs::{FileSystem, FileSystemPath};
use nxpkgpack::{
...
use nxpkgpack_dev::react_refresh::assert_can_resolve_react_refresh;
...
pub async fn get_client_import_map(project_path: Vc<FileSystemPath>) -> Result<Vc<ImportMap>> {
    ...
    import_map.insert_wildcard_alias(
        "@vercel/nxpkgpack-ecmascript-runtime/",
        ImportMapping::PrimaryAlternative(
            "./*".to_string(),
            Some(nxpkgpack_ecmascript_runtime::embed_fs().root()),
        )
        .cell(),
    );
