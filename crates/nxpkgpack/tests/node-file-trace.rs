...
use nxpkg_tasks::{backend::Backend, ReadRef, NxpkgTasks, Value, ValueToString, Vc};
use nxpkg_tasks_fs::{DiskFileSystem, FileSystem, FileSystemPath};
...
fn node_file_trace_memory(#[case] input: CaseInput) {
    node_file_trace(
        ...
        1,
        120,
        |_| NxpkgTasks::new(MemoryBackend::default()),
        |tt| {
            let b = tt.backend();
...
fn node_file_trace_rocksdb(#[case] input: CaseInput) {
    ...
    node_file_trace(
        ...
        240,
        |directory_path| {
            NxpkgTasks::new(MemoryBackendWithPersistedGraph::new(
...
fn node_file_trace<B: Backend + 'static>(
    ...
    run_count: i32,
    timeout_len: u64,
    create_nxpkg_tasks: impl Fn(&Path) -> Arc<NxpkgTasks<B>>,
    handle_timeout_error: impl Fn(&Arc<NxpkgTasks<B>>),
) {
    lazy_static! {
    ...
    r.block_on(async move {
        ...
        for _ in 0..run_count {
            ...
            let tt = create_nxpkg_tasks(directory_path.as_path());
