...
use nxpkg_tasks_hash::{hash_xxh3_hash64, DeterministicHash, DeterministicHasher};
...
#[nxpkg_tasks::value(cell = "new", eq = "manual")]
...
impl DiskFileSystem {
    ...
    /// has to be called within a nxpkg-tasks function
    fn register_invalidator(&self, path: &Path) -> Result<()> {
        let invalidator = nxpkg_tasks::get_invalidator();
    ...
    /// registers the path as an invalidator for the current task,
    /// has to be called within a nxpkg-tasks function. It removes and returns
...
impl FileSystem for DiskFileSystem {
    ...
    async fn write(
        ...
        // be freed immediately. Given this is an output file, it's unlikely any Nxpkg
...
// Not nxpkg-tasks functions, only delegating
impl FileSystemPath {
...
#[nxpkg_tasks::value(shared, serialization = "none")]
...
pub enum FileLinesContent {
    Lines(#[nxpkg_tasks(trace_ignore)] Vec<FileLine>),
    Unparseable,
    NotFound,
}
...
mod tests {
    ...
    async fn with_extension() {
        ...
        nxpkg_tasks_testing::VcStorage::with(async {
