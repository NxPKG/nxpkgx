...
use thiserror::Error;
use nxpkgpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf, RelativeUnixPath};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err = match self.package_manager {
            PackageManager::Pnpm | PackageManager::Pnpm6 => {
                "pnpm-workspace.yaml: no packages found. Nxpkgrepo requires pnpm workspaces and \
                 thus packages to be defined in the root pnpm-workspace.yaml"
            }
            PackageManager::Yarn | PackageManager::Berry => {
                "package.json: no workspaces found. Nxpkgrepo requires yarn workspaces to be \
                 defined in the root package.json"
            }
            PackageManager::Npm => {
                "package.json: no workspaces found. Nxpkgrepo requires npm workspaces to be \
                 defined in the root package.json"
            }
            PackageManager::Bun => {
                "package.json: no workspaces found. Nxpkgrepo requires bun workspaces to be \
                 defined in the root package.json"
            }
        };
        write!(f, "{}", err)
...
impl PackageManager {
    ...
    pub fn parse_lockfile(
        ...
        Ok(match self {
            PackageManager::Npm => Box::new(nxpkgrepo_lockfiles::NpmLockfile::load(contents)?),
            PackageManager::Pnpm | PackageManager::Pnpm6 => {
                Box::new(nxpkgrepo_lockfiles::PnpmLockfile::from_bytes(contents)?)
            }
            PackageManager::Yarn => {
                Box::new(nxpkgrepo_lockfiles::Yarn1Lockfile::from_bytes(contents)?)
            }
            PackageManager::Bun => {
                Box::new(nxpkgrepo_lockfiles::BunLockfile::from_bytes(contents)?)
            }
            PackageManager::Berry => Box::new(nxpkgrepo_lockfiles::BerryLockfile::load(
                contents,
                Some(nxpkgrepo_lockfiles::BerryManifest::with_resolutions(
                    root_package_json
                        .resolutions
                        .iter()
                        .flatten()
    ...
    pub fn lockfile_path(&self, nxpkg_root: &AbsoluteSystemPath) -> AbsoluteSystemPathBuf {
        nxpkg_root.join_component(self.lockfile_name())
    }
...
mod tests {
    ...
    fn repo_root() -> AbsoluteSystemPathBuf {
        ...
        panic!("Couldn't find Nxpkgrepo root from {}", cwd);
    }
