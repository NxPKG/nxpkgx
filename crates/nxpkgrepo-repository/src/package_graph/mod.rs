...
use serde::Serialize;
use nxpkgpath::{AbsoluteSystemPath, AnchoredSystemPath, AnchoredSystemPathBuf};
...
pub struct WorkspaceInfo {
    ...
    pub transitive_dependencies: Option<HashSet<nxpkgrepo_lockfiles::Package>>,
}
...
impl PackageGraph {
    ...
    pub fn transitive_external_dependencies<'a, I: IntoIterator<Item = &'a WorkspaceName>>(
        &self,
        workspaces: I,
    ) -> HashSet<&nxpkgrepo_lockfiles::Package> {
        workspaces
            .into_iter()
    ...
    pub fn changed_packages(
        ...
        let closures = nxpkgrepo_lockfiles::all_transitive_closures(previous, external_deps)?;
...
mod test {
    ...
    struct MockLockfile {}
    impl nxpkgrepo_lockfiles::Lockfile for MockLockfile {
        fn resolve_package(
            &self,
            _workspace_path: &str,
            name: &str,
            _version: &str,
        ) -> std::result::Result<Option<nxpkgrepo_lockfiles::Package>, nxpkgrepo_lockfiles::Error>
        {
            Ok(match name {
                "a" => Some(nxpkgrepo_lockfiles::Package::new("key:a", "1")),
                "b" => Some(nxpkgrepo_lockfiles::Package::new("key:b", "1")),
                "c" => Some(nxpkgrepo_lockfiles::Package::new("key:c", "1")),
                _ => None,
            })
        }
        ...
        fn all_dependencies(
            &self,
            key: &str,
        ) -> std::result::Result<Option<HashMap<String, String>>, nxpkgrepo_lockfiles::Error>
        {
            match key {
                "key:a" => Ok(Some(
                    [("c", "1")]
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> std::result::Result<Box<dyn Lockfile>, nxpkgrepo_lockfiles::Error> {
            unreachable!("lockfile pruning not necessary for package graph construction")
        }

        fn encode(&self) -> std::result::Result<Vec<u8>, nxpkgrepo_lockfiles::Error> {
    ...
    fn test_lockfile_traversal() {
        ...
        let bar_deps = pkg_graph
            ...
            .as_ref()
            .unwrap();
        let a = nxpkgrepo_lockfiles::Package::new("key:a", "1");
        let b = nxpkgrepo_lockfiles::Package::new("key:b", "1");
        let c = nxpkgrepo_lockfiles::Package::new("key:c", "1");
