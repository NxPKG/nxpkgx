...
macro_rules! duration_span {
    ($name:literal) => {
        nxpkg_tasks::duration_span::DurationSpanGuard::new(|duration| {
            nxpkg_tasks::macro_helpers::tracing::info!(name = $name, duration = duration);
        })
    };
    ($name:literal, $($arg:tt)+) => {
        nxpkg_tasks::duration_span::DurationSpanGuard::new(|duration| {
            nxpkg_tasks::macro_helpers::tracing::info!(name = $name, $($arg)+, duration = duration);
        })
    };
}
