...
use crate::{
    backend::CellContent,
    event::EventListener,
    manager::{read_task_cell, read_task_output, NxpkgTasksApi},
    registry::{
        get_value_type, {self},
    },
    nxpkg_tasks,
...
impl RawVc {
    ...
    pub(crate) fn into_read_untracked_with_nxpkg_tasks<T: Any + VcValueType>(
        self,
        nxpkg_tasks: &dyn NxpkgTasksApi,
    ) -> ReadRawVcFuture<T, VcValueTypeCast<T>> {
        ReadRawVcFuture::new_untracked_with_nxpkg_tasks(self, nxpkg_tasks)
    }
...
pub struct ReadRawVcFuture<T: ?Sized, Cast = VcValueTypeCast<T>> {
    nxpkg_tasks: Arc<dyn NxpkgTasksApi>,
...
impl<T: ?Sized, Cast: VcCast> ReadRawVcFuture<T, Cast> {
    ...
    fn new_untracked_with_nxpkg_tasks(vc: RawVc, nxpkg_tasks: &dyn NxpkgTasksApi) -> Self {
        let tt = nxpkg_tasks.pin();
        ReadRawVcFuture {
            nxpkg_tasks: tt,
            strongly_consistent: false,
            current: vc,
...
impl<T: ?Sized, Cast: VcCast> Future for ReadRawVcFuture<T, Cast> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        self.nxpkg_tasks.notify_scheduled_tasks();
        ...
        'outer: loop {
            ...
            let mut listener = match this.current {
                ...
                RawVc::TaskCell(task, index) => {
                    let read_result = if this.untracked {
                        this.nxpkg_tasks.try_read_task_cell_untracked(task, index)
                    } else {
                        this.nxpkg_tasks.try_read_task_cell(task, index)
                    };
                    match read_result {
                        Ok(Ok(content)) => {
