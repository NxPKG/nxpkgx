...
use crate::{
    event::EventListener, manager::NxpkgTasksBackendApi, raw_vc::CellId, registry,
...
pub trait Backend: Sync + Send {
    ...
    #[allow(unused_variables)]
    fn startup(&self, nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>) {}

    #[allow(unused_variables)]
    fn stop(&self, nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>) {}

    #[allow(unused_variables)]
    fn idle_start(&self, nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>) {}

    fn invalidate_task(&self, task: TaskId, nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>);

    fn invalidate_tasks(&self, tasks: &[TaskId], nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>);
    fn invalidate_tasks_set(&self, tasks: &TaskIdSet, nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>);
    ...
    fn try_start_task_execution(
        &self,
        task: TaskId,
        nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>,
    ...
    fn run_backend_job<'a>(
        &'a self,
        id: BackendJobId,
        nxpkg_tasks: &'a dyn NxpkgTasksBackendApi<Self>,
    ...
    fn try_read_own_task_cell_untracked(
        ...
        match self.try_read_task_cell_untracked(current_task, index, nxpkg_tasks)? {
    ...
    fn mark_own_task_as_finished(
        &self,
        _task: TaskId,
        _nxpkg_tasks: &dyn NxpkgTasksBackendApi<Self>,
    ) {
...
impl PersistentTaskType {
    pub async fn run_resolve_native<B: Backend + 'static>(
        ...
        nxpkg_tasks: Arc<dyn NxpkgTasksBackendApi<B>>,
    ) -> Result<RawVc> {
        let span = tracing::trace_span!(
            "nxpkg_tasks::resolve_call",
        ...
        async move {
            ...
            Ok(nxpkg_tasks.native_call(fn_id, inputs))
        }
        .instrument(span)
        .await
    }
    ...
    pub async fn run_resolve_trait<B: Backend + 'static>(
        ...
        let span = tracing::trace_span!(
            "nxpkg_tasks::resolve_trait_call",
            ...
            if let Some(this) = iter.next() {
                ...
                match this_value.get_trait_method(trait_type, name) {
                    Ok(native_fn) => {
                        ...
                        Ok(nxpkg_tasks.dynamic_call(native_fn, resolved_inputs))
                    }
                    Err(name) => {
    ...
    pub fn run<B: Backend + 'static>(
        ...
        match self {
            ...
            PersistentTaskType::ResolveNative(fn_id, inputs) => {
                Box::pin(Self::run_resolve_native(fn_id, inputs, nxpkg_tasks))
            }
            PersistentTaskType::ResolveTrait(trait_type, name, inputs) => Box::pin(
                Self::run_resolve_trait(trait_type, name, inputs, nxpkg_tasks),
            ),
        }
    }
}
