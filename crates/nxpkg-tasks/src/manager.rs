...
pub trait NxpkgTasksCallApi: Sync + Send {
...
pub trait NxpkgTasksApi: NxpkgTasksCallApi + Sync + Send {
    fn pin(&self) -> Arc<dyn NxpkgTasksApi>;
...
pub enum StatsType {
    /// Only report stats essential to Nxpkg Tasks' operation.
    Essential,
...
pub trait NxpkgTasksBackendApi<B: Backend + 'static>:
    TaskIdProvider + NxpkgTasksCallApi + Sync + Send
{
    fn pin(&self) -> Arc<dyn NxpkgTasksBackendApi<B>>;
...
impl<B: Backend + 'static> TaskIdProvider for &dyn NxpkgTasksBackendApi<B> {
    fn get_fresh_task_id(&self) -> Unused<TaskId> {
...
pub struct NxpkgTasks<B: Backend + 'static> {
    this: Weak<Self>,
    backend: B,
...
task_local! {
    /// The current NxpkgTasks instance
    static NXPKG_TASKS: Arc<dyn NxpkgTasksApi>;
...
impl<B: Backend + 'static> NxpkgTasks<B> {
    // TODO better lifetime management for nxpkg tasks
    // consider using unsafe for the task_local nxpkg tasks
    // that should be safe as long tasks can't outlife nxpkg task
    // so we probably want to make sure that all tasks are joined
    // when trying to drop nxpkg tasks
    pub fn new(mut backend: B) -> Arc<Self> {
        ...
        let this = Arc::new_cyclic(|this| Self {
            ...
            event: Event::new(|| "NxpkgTasks::event".to_string()),
            event_start: Event::new(|| "NxpkgTasks::event_start".to_string()),
            event_foreground: Event::new(|| "NxpkgTasks::event_foreground".to_string()),
            event_background: Event::new(|| "NxpkgTasks::event_background".to_string()),
    ...
    pub async fn run_once<T: TraceRawVcs + Send + 'static>(
        ...
        raw_result
            .into_read_untracked_with_nxpkg_tasks::<Completion>(self)
            .await?;
    ...
    pub(crate) fn native_call(&self, func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
        RawVc::TaskOutput(self.backend.get_or_create_persistent_task(
            ...
            current_task("nxpkg_function calls"),
            self,
        ))
    }
    ...
    pub(crate) fn schedule_background_job<
        T: FnOnce(Arc<NxpkgTasks<B>>) -> F + Send + 'static,
...
impl<B: Backend + 'static> NxpkgTasksCallApi for NxpkgTasks<B> {
    fn dynamic_call(&self, func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
...
impl<B: Backend + 'static> NxpkgTasksApi for NxpkgTasks<B> {
    fn pin(&self) -> Arc<dyn NxpkgTasksApi> {
        self.pin()
    }
...
impl<B: Backend + 'static> NxpkgTasksBackendApi<B> for NxpkgTasks<B> {
    fn pin(&self) -> Arc<dyn NxpkgTasksBackendApi<B>> {
        self.pin()
    }
    fn backend(&self) -> &B {
...
impl<B: Backend + 'static> TaskIdProvider for NxpkgTasks<B> {
    fn get_fresh_task_id(&self) -> Unused<TaskId> {
        // Safety: This is a fresh id from the factory
...
pub(crate) fn current_task(from: &str) -> TaskId {
    match CURRENT_TASK_ID.try_with(|id| *id) {
        Ok(id) => id,
        Err(_) => panic!(
            "{} can only be used in the context of nxpkg_tasks task execution",
            from
        ),
    }
}
...
pub struct Invalidator {
    task: TaskId,
    nxpkg_tasks: Weak<dyn NxpkgTasksApi>,
    handle: Handle,
}
...
impl Invalidator {
    pub fn invalidate(self) {
        ...
        let _ = handle.enter();
        if let Some(nxpkg_tasks) = nxpkg_tasks.upgrade() {
            nxpkg_tasks.invalidate(task);
        }
    }
    ...
    pub fn invalidate_with_reason<T: InvalidationReason>(self, reason: T) {
        ...
        let _ = handle.enter();
        if let Some(nxpkg_tasks) = nxpkg_tasks.upgrade() {
            nxpkg_tasks.invalidate_with_reason(
                task,
...
pub async fn run_once<T: Send + 'static>(
    ...
    raw_result
        .into_read_untracked_with_nxpkg_tasks::<Completion>(&*tt)
        .await?;
...
/// see [NxpkgTasks] `dynamic_call`
pub fn dynamic_call(func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
    with_nxpkg_tasks(|tt| tt.dynamic_call(func, inputs))
}

/// see [NxpkgTasks] `trait_call`
pub fn trait_call(
    trait_type: TraitTypeId,
    trait_fn_name: Cow<'static, str>,
    inputs: Vec<ConcreteTaskInput>,
) -> RawVc {
    with_nxpkg_tasks(|tt| tt.trait_call(trait_type, trait_fn_name, inputs))
}

pub fn nxpkg_tasks() -> Arc<dyn NxpkgTasksApi> {
    NXPKG_TASKS.with(|arc| arc.clone())
}

pub fn with_nxpkg_tasks<T>(func: impl FnOnce(&Arc<dyn NxpkgTasksApi>) -> T) -> T {
    NXPKG_TASKS.with(|arc| func(arc))
}

pub fn weak_nxpkg_tasks() -> Weak<dyn NxpkgTasksApi> {
    NXPKG_TASKS.with(Arc::downgrade)
}

pub fn with_nxpkg_tasks_for_testing<T>(
    tt: Arc<dyn NxpkgTasksApi>,
    current_task: TaskId,
...
pub fn spawn_detached(f: impl Future<Output = Result<()>> + Send + 'static) {
    tokio::spawn(nxpkg_tasks().detached(Box::pin(f.in_current_span())));
}
...
pub fn get_invalidator() -> Invalidator {
    ...
    Invalidator {
        task: current_task("nxpkg_tasks::get_invalidator()"),
        nxpkg_tasks: weak_nxpkg_tasks(),
        handle,
    }
}
...
pub fn mark_finished() {
    with_nxpkg_tasks(|tt| {
        tt.mark_own_task_as_finished(current_task("nxpkg_tasks::mark_finished()"))
    });
}
...
pub fn notify_scheduled_tasks() {
    with_nxpkg_tasks(|tt| tt.notify_scheduled_tasks())
}

pub fn emit<T: VcValueTrait + Send>(collectible: Vc<T>) {
    with_nxpkg_tasks(|tt| tt.emit_collectible(T::get_trait_type_id(), collectible.node))
}
...
pub fn find_cell_by_type(ty: ValueTypeId) -> CurrentCellRef {
    CELL_COUNTERS.with(|cell| {
        let current_task = current_task("celling nxpkg_tasks values");
