...
use nxpkg_tasks::{Completion, TryJoinIterExt, NxpkgTasks, Value, Vc};
use nxpkg_tasks_bytes::stream::SingleValue;
use nxpkg_tasks_env::CommandLineProcessEnv;
use nxpkg_tasks_fs::{
...
use nxpkgpack_node::{debug::should_debug, evaluate::evaluate};
use nxpkgpack_test_utils::jest::JestRunResult;
...
async fn run_test(resource: String) -> Result<Vc<RunTestResult>> {
    ...
    let tests_path = project_fs.root().join("crates/nxpkgpack-tests".to_string());
...
async fn snapshot_issues(run_result: Vc<RunTestResult>) -> Result<Vc<()>> {
    ...
    nxpkgpack_test_utils::snapshot::snapshot_issues(
        plain_issues,
