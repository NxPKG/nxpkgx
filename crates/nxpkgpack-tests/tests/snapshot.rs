...
use serde_json::json;
use nxpkg_tasks::{ReadRef, TryJoinIterExt, NxpkgTasks, Value, ValueToString, Vc};
use nxpkg_tasks_env::DotenvProcessEnv;
use nxpkg_tasks_fs::{
...
use nxpkgpack_ecmascript_plugins::transform::{
...
use nxpkgpack_ecmascript_runtime::RuntimeType;
use nxpkgpack_env::ProcessEnvAsset;
use nxpkgpack_test_utils::snapshot::{diff, expected, matches_expected, snapshot_issues};

use crate::util::REPO_ROOT;

fn register() {
    nxpkg_tasks::register();
    nxpkg_tasks_env::register();
    nxpkg_tasks_fs::register();
    nxpkgpack::register();
    nxpkgpack_build::register();
    nxpkgpack_dev::register();
    nxpkgpack_env::register();
...
async fn run_test(resource: String) -> Result<Vc<FileSystemPath>> {
    ...
    let defines = compile_time_defines!(
        process.nxpkgpack = true,
        process.env.NXPKGPACK = true,
