...
use nxpkg_tasks::{TryJoinIterExt, NxpkgTasks, Vc};
...
pub fn fibonacci(c: &mut Criterion) {
    if matches!(
        std::env::var("NXPKGPACK_BENCH_STRESS").ok().as_deref(),
    ...
    let mut group = c.benchmark_group("nxpkg_tasks_memory_stress");
...
/// Computes a fibonacci number in a recursive matter. Due to nxpkg-tasks this
/// will result in a lot of cached tasks, so its performance is only O(n)
/// (compared to non-nxpkg-tasks O(1.6^N)).
