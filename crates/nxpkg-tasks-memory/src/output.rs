...
use nxpkg_tasks::{util::SharedError, RawVc, TaskId, TaskIdSet, NxpkgTasksBackendApi};
...
impl Output {
    ...
    pub fn link(&mut self, target: RawVc, nxpkg_tasks: &dyn NxpkgTasksBackendApi<MemoryBackend>) {
        debug_assert!(*self != target);
        self.assign(OutputContent::Link(target), nxpkg_tasks)
    }

    pub fn error(&mut self, error: Error, nxpkg_tasks: &dyn NxpkgTasksBackendApi<MemoryBackend>) {
        self.content = OutputContent::Error(SharedError::new(error));
        self.updates += 1;
        // notify
        if !self.dependent_tasks.is_empty() {
            nxpkg_tasks.schedule_notify_tasks_set(&take(&mut self.dependent_tasks));
        }
    }
    ...
    pub fn gc_drop(self, nxpkg_tasks: &dyn NxpkgTasksBackendApi<MemoryBackend>) {
        // notify
        if !self.dependent_tasks.is_empty() {
            nxpkg_tasks.schedule_notify_tasks_set(&self.dependent_tasks);
        }
    }
}
