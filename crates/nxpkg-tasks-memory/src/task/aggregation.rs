...
use nxpkg_tasks::{event::Event, RawVc, TaskId, TaskIdSet, TraitTypeId, NxpkgTasksBackendApi};
...
pub struct TaskAggregationContext<'a> {
    pub nxpkg_tasks: &'a dyn NxpkgTasksBackendApi<MemoryBackend>,
    ...
    pub fn apply_queued_updates(&mut self) {
        {
            ...
            if let Some(tasks) = tasks.as_mut() {
                ...
                if !tasks.is_empty() {
                    ...
                    self.backend
                        .schedule_when_dirty_from_aggregation(tasks, self.nxpkg_tasks);
                }
            }
        }
        ...
        if let Some(tasks) = tasks.as_mut() {
            let tasks = take(tasks);
            if !tasks.is_empty() {
                self.nxpkg_tasks.schedule_notify_tasks_set(&tasks);
            }
        }
    }
