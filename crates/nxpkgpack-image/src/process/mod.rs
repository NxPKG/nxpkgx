...
use nxpkg_tasks::{debug::ValueDebugFormat, trace::TraceRawVcs, Vc};
use nxpkg_tasks_fs::{File, FileContent, FileSystemPath};
use nxpkgpack_core::{
...
fn load_image_internal(
    ...
    // [NOTE]
    // Workaround for missing codec supports in Nxpkgpack,
    ...
    if matches!(format, Some(ImageFormat::Avif)) {
        ImageProcessingIssue {
            ...
            message: StyledString::Text(
                "This version of Nxpkgpack does not support AVIF images, will emit without \
    ...
    if matches!(format, Some(ImageFormat::WebP)) {
        ImageProcessingIssue {
            path: ident.path(),
            message: StyledString::Text(
                "This version of Nxpkgpack does not support WEBP images, will emit without \
                 optimization or encoding"
                    .to_string(),
            )
            .cell(),
