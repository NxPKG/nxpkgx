declare const __nxpkgpack_external_require__: {
...
try {
  ({ runLoaders } = require("@vercel/nxpkgpack/loader-runner"));
} catch {
  ({ runLoaders } = __nxpkgpack_external_require__("loader-runner"));
}
    ...
    runLoaders(
      {
        ...
        loaders: loadersWithOptions.map((loader) => ({
          loader: __nxpkgpack_external_require__.resolve(loader.loader, {
            paths: [resourceDir],
          }),
