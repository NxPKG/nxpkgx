...
use nxpkg_tasks::{duration_span, mark_finished, util::SharedError, RawVc, ValueToString, Vc};
use nxpkg_tasks_bytes::{Bytes, Stream};
...
use nxpkgpack_dev_server::source::{Body, ProxyResult};
...
#[nxpkg_tasks::value(eq = "manual", cell = "new", serialization = "none")]
struct RenderStreamSender {
    #[nxpkg_tasks(trace_ignore, debug_ignore)]
    get: Box<dyn Fn() -> UnboundedSender<RenderItemResult> + Send + Sync>,
}

#[nxpkg_tasks::value(transparent)]
struct RenderStream(#[nxpkg_tasks(trace_ignore)] Stream<RenderItemResult>);
...
fn render_stream(
    ...
    let cell = nxpkg_tasks::macro_helpers::find_cell_by_type(*RENDERSTREAM_VALUE_TYPE_ID);
