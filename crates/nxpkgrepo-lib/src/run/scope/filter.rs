...
use tracing::debug;
use nxpkgpath::{AbsoluteSystemPath, AnchoredSystemPathBuf};
use nxpkgrepo_repository::package_graph::{self, PackageGraph, WorkspaceName};
...
impl PackageInference {
    // calculate, based on the directory that global nxpkg was invoked in,
    // the pieces of a filter spec that we will infer. If nxpkg was invoked
    // somewhere between the root and packages, scope nxpkg invocations to the
    // packages below where nxpkg was invoked. If nxpkg was invoked at or within
    // a particular package, scope the nxpkg invocation to just that package.
    pub fn calculate(
        ...
        let full_inference_path = nxpkg_root.resolve(pkg_inference_path);
        for (workspace_name, workspace_entry) in pkg_graph.workspaces() {
            let pkg_path = nxpkg_root.resolve(workspace_entry.package_json_path());
            ...
            if inferred_path_is_below && (&pkg_path as &AbsoluteSystemPath) != nxpkg_root {
    ...
    pub fn apply(&self, selector: &mut TargetSelector) {
        ...
        if selector.parent_dir != nxpkgpath::AnchoredSystemPathBuf::default() {
...
pub struct FilterResolver<'a, T: PackageChangeDetector> {
    ...
    nxpkg_root: &'a AbsoluteSystemPath,
...
impl<'a> FilterResolver<'a, SCMChangeDetector<'a>> {
    pub(crate) fn new(
        ...
        Self::new_with_change_detector(pkg_graph, nxpkg_root, inference, scm, change_detector)
    }
}
...
mod test {
    ...
    use nxpkgpath::{AbsoluteSystemPathBuf, AnchoredSystemPathBuf, RelativeUnixPathBuf};
    ...
    fn make_project<T: PackageChangeDetector>(
        ...
        let nxpkg_root = Box::leak(Box::new(
        ...
        let pkg_graph = Box::leak(Box::new(
            PackageGraph::builder(nxpkg_root, Default::default())
        ...
        let scm = Box::leak(Box::new(nxpkgrepo_scm::SCM::new(nxpkg_root)));
