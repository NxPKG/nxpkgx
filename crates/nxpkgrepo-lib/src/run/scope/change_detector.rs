...
use nxpkgrepo_repository::package_graph::{ChangedPackagesError, PackageGraph, WorkspaceName};
...
impl<'a> PackageChangeDetector for SCMChangeDetector<'a> {
    fn changed_packages(
        ...
        if !from_ref.is_empty() {
            changed_files = self
                .scm
                .changed_files(self.nxpkg_root, Some(from_ref), to_ref)?;
        }
...
impl<'a> SCMChangeDetector<'a> {
    const DEFAULT_GLOBAL_DEPS: [&'static str; 2] = ["package.json", "nxpkg.json"];
    ...
    ) -> Result<bool, nxpkgrepo_scm::Error> {
    ...
    ) -> Result<HashSet<&'b AnchoredSystemPathBuf>, nxpkgrepo_scm::Error> {
    ...
    fn get_changed_packages<'b>(
        ...
        graph: &PackageGraph,
    ) -> Result<HashSet<WorkspaceName>, nxpkgrepo_scm::Error> {
    ...
    fn get_changes_from_lockfile(
        ...
        let lockfile_path = self
            .pkg_graph
            .package_manager()
            .lockfile_path(self.nxpkg_root);

        // no changes to lockfile? then no changes because of lockfile
        if !SCMChangeDetector::lockfile_changed(self.nxpkg_root, changed_files, &lockfile_path)? {
            return Ok(vec![]);
        }
    ...
    fn lockfile_changed(
        ...
        let lockfile_path_relative = nxpkg_root
...
pub enum ChangeDetectError {
    ...
    Lockfile(nxpkgrepo_lockfiles::Error),
}
...
mod test {
    ...
    fn test_lockfile_changed(
        ...
        let nxpkg_root = nxpkgpath::AbsoluteSystemPathBuf::new(nxpkg_root).unwrap();
        let lockfile_path = nxpkgpath::AbsoluteSystemPathBuf::new(lockfile_path).unwrap();
        let changed_files = changed_files
            .iter()
            .map(|s| nxpkgpath::AnchoredSystemPathBuf::from_raw(s).unwrap())
            .collect();
        let changes =
            SCMChangeDetector::lockfile_changed(&nxpkg_root, &changed_files, &lockfile_path);
