...
impl SpacesClient {
    pub fn new(
        ...
        if !is_linked {
            eprintln!(
                "Error: experimentalSpaceId is enabled, but repo is not linked to API. Run `nxpkg \
                 link` or `nxpkg login` first"
            );
            return None;
        }
    ...
    fn from(value: SpacesTaskInformation) -> Self {
        ...
        let cache = cache_status.map_or_else(
            ...
            |CacheHitMetadata { source, time_saved }| SpacesCacheStatus {
                status: nxpkgrepo_api_client::spaces::CacheStatus::Hit,
                source: Some(match source {
                    nxpkgrepo_cache::CacheSource::Local => {
                        nxpkgrepo_api_client::spaces::CacheSource::Local
                    }
                    nxpkgrepo_cache::CacheSource::Remote => {
                        nxpkgrepo_api_client::spaces::CacheSource::Remote
                    }
                }),
                time_saved,
            },
