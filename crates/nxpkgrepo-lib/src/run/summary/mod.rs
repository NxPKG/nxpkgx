...
use nxpkgpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf, AnchoredSystemPath};
use nxpkgrepo_api_client::{spaces::CreateSpaceRunPayload, APIAuth, APIClient};
use nxpkgrepo_env::EnvironmentVariableMap;
use nxpkgrepo_repository::package_graph::{PackageGraph, WorkspaceName};
use nxpkgrepo_ui::{color, cprintln, cwriteln, BOLD, BOLD_CYAN, GREY, UI};
...
pub enum Error {
    ...
    SpacesRequest(#[from] nxpkgrepo_api_client::Error),
    ...
    Env(#[source] nxpkgrepo_env::Error),
...
// for rendering a single-package run of nxpkg. Notably, we want to always omit
...
impl<'a> From<&'a RunSummary<'a>> for SinglePackageRunSummary<'a> {
    fn from(run_summary: &'a RunSummary<'a>) -> Self {
        ...
        SinglePackageRunSummary {
            ...
            nxpkg_version: run_summary.nxpkg_version,
...
impl<'a> RunSummary<'a> {
    ...
    async fn send_to_space(
        ...
        let spinner = nxpkgrepo_ui::start_spinner("...sending run summary...");
    ...
    fn get_path(&self) -> AbsoluteSystemPathBuf {
        ...
        self.repo_root
            .join_components(&[".nxpkg", "runs", &filename])
    }
