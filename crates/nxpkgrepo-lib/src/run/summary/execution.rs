...
use serde::Serialize;
use tokio::sync::mpsc;
use nxpkgpath::{AbsoluteSystemPathBuf, AnchoredSystemPath};
use nxpkgrepo_ui::{color, cprintln, BOLD, BOLD_GREEN, BOLD_RED, MAGENTA, UI, YELLOW};
...
pub struct ExecutionSummary<'a> {
    // a synthesized nxpkg command to produce this invocation
    command: String,
    // the (possibly empty) path from the nxpkgrepo root to where the command was run
    ...
    #[serde(skip)]
    duration: NxpkgDuration,
    pub(crate) exit_code: i32,
}

#[derive(Debug)]
struct NxpkgDuration(Duration);
...
impl From<Duration> for NxpkgDuration {
    fn from(duration: Duration) -> Self {
        Self(duration)
    }
}

impl fmt::Display for NxpkgDuration {
...
impl<'a> ExecutionSummary<'a> {
    pub fn new(
        ...
        let duration = NxpkgDuration::new(&start_time, &end_time);
        Self {
            command,
    ...
    pub fn print(&self, ui: UI, path: AbsoluteSystemPathBuf, failed_tasks: Vec<&TaskSummary>) {
        let maybe_full_nxpkg = if self.cached == self.attempted && self.attempted > 0 {
            match std::env::var("TERM_PROGRAM").as_deref() {
                Ok("Apple_Terminal") => color!(ui, MAGENTA, ">>> FULL NXPKG").to_string(),
                _ => ui.rainbow(">>> FULL NXPKG").to_string(),
            }
        } else {
            String::new()
        };
