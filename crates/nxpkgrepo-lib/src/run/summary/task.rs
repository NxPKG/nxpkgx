...
use serde::Serialize;
use nxpkgpath::{AnchoredSystemPathBuf, RelativeUnixPathBuf};
use nxpkgrepo_cache::CacheHitMetadata;
use nxpkgrepo_env::{DetailedMap, EnvironmentVariableMap};
...
impl From<nxpkgrepo_cache::CacheSource> for CacheSource {
    fn from(value: nxpkgrepo_cache::CacheSource) -> Self {
        match value {
            nxpkgrepo_cache::CacheSource::Local => Self::Local,
            nxpkgrepo_cache::CacheSource::Remote => Self::Remote,
        }
    }
}
...
impl TaskEnvVarSummary {
    pub fn new(
    ...
    ) -> Result<Self, nxpkgrepo_env::Error> {
        ...
        let pass_through = task_definition
            .pass_through_env
            .as_deref()
            .map(|pass_through_env| -> Result<_, nxpkgrepo_env::Error> {
