...
use nxpkgrepo_repository::package_json::{Error as PackageJsonError, PackageJson};

use crate::{
    commands::CommandBase,
    config::{Error as ConfigError, RawNxpkgJSON},
};
...
#[derive(Default)]
pub struct NxpkgrepoConfigBuilder {
...
impl ResolvedConfigurationOptions for PackageJson {
    fn get_configuration_options(self) -> Result<ConfigurationOptions, ConfigError> {
        match &self.legacy_nxpkg_config {
            Some(legacy_nxpkg_config) => {
                let synthetic_raw_nxpkg_json: RawNxpkgJSON =
                    serde_json::from_value(legacy_nxpkg_config.clone())?;
                synthetic_raw_nxpkg_json.get_configuration_options()
            }
...
impl ResolvedConfigurationOptions for RawNxpkgJSON {
    fn get_configuration_options(self) -> Result<ConfigurationOptions, ConfigError> {
...
fn get_env_var_config(
    ...
    let mut nxpkg_mapping = HashMap::new();
    nxpkg_mapping.insert(OsString::from("nxpkg_api"), "api_url");
    nxpkg_mapping.insert(OsString::from("nxpkg_login"), "login_url");
    nxpkg_mapping.insert(OsString::from("nxpkg_team"), "team_slug");
    nxpkg_mapping.insert(OsString::from("nxpkg_teamid"), "team_id");
    nxpkg_mapping.insert(OsString::from("nxpkg_token"), "token");
    nxpkg_mapping.insert(OsString::from("nxpkg_remote_cache_timeout"), "timeout");

    // We do not enable new config sources:
    // nxpkg_mapping.insert(String::from("nxpkg_signature"), "signature"); // new
    // nxpkg_mapping.insert(String::from("nxpkg_preflight"), "preflight"); // new
    // nxpkg_mapping.insert(String::from("nxpkg_remote_cache_enabled"), "enabled");

    let mut output_map = HashMap::new();

    nxpkg_mapping.into_iter().try_for_each(
...
impl NxpkgrepoConfigBuilder {
    ...
    fn global_config_path(&self) -> Result<AbsoluteSystemPathBuf, ConfigError> {
        ...
        let global_config_path = config_dir.join("nxpkgrepo").join("config.json");
    ...
    fn local_config_path(&self) -> AbsoluteSystemPathBuf {
        self.repo_root.join_components(&[".nxpkg", "config.json"])
    }
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    fn root_nxpkg_json_path(&self) -> AbsoluteSystemPathBuf {
        self.repo_root.join_component("nxpkg.json")
    }
    ...
    pub fn build(&self) -> Result<ConfigurationOptions, ConfigError> {
        ...
        // - shared configuration (package.json .nxpkg)
        // - shared configuration (nxpkg.json)
        // - global configuration (~/.nxpkg/config.json)
        // - local configuration (<REPO_ROOT>/.nxpkg/config.json)
        ...
        let nxpkg_json =
            RawNxpkgJSON::read(&self.repo_root.join_component("nxpkg.json")).or_else(|e| {
        ...
        let sources = [
            ...
            nxpkg_json.get_configuration_options(),
...
mod test {
    ...
    fn test_env_setting() {
        ...
        let nxpkg_api = "https://example.com/api";
        let nxpkg_login = "https://example.com/login";
        let nxpkg_team = "vercel";
        let nxpkg_teamid = "team_nLlpyC6REAqxydlFKbrMDlud";
        let nxpkg_token = "abcdef1234567890abcdef";
        let nxpkg_remote_cache_timeout = 200;

        env.insert("nxpkg_api".into(), nxpkg_api.into());
        env.insert("nxpkg_login".into(), nxpkg_login.into());
        env.insert("nxpkg_team".into(), nxpkg_team.into());
        env.insert("nxpkg_teamid".into(), nxpkg_teamid.into());
        env.insert("nxpkg_token".into(), nxpkg_token.into());
        env.insert(
            "nxpkg_remote_cache_timeout".into(),
            nxpkg_remote_cache_timeout.to_string().into(),
        );

        let config = get_env_var_config(&env).unwrap();
        assert_eq!(nxpkg_api, config.api_url.unwrap());
        assert_eq!(nxpkg_login, config.login_url.unwrap());
        assert_eq!(nxpkg_team, config.team_slug.unwrap());
        assert_eq!(nxpkg_teamid, config.team_id.unwrap());
        assert_eq!(nxpkg_token, config.token.unwrap());
        assert_eq!(nxpkg_remote_cache_timeout, config.timeout.unwrap());
    }
    ...
    fn test_env_layering() {
        ...
        let builder = NxpkgrepoConfigBuilder {
            repo_root,
            override_config,
    ...
    fn test_shared_no_token() {
        let mut test_shared_config: RawNxpkgJSON = Default::default();
