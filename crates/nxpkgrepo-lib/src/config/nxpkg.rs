...
use nxpkgrepo_repository::{package_graph::ROOT_PKG_NAME, package_json::PackageJson};
...
// The processed NxpkgJSON ready for use by Nxpkgrepo.
pub struct NxpkgJson {
...
#[serde(rename_all = "camelCase")]
// The raw deserialized nxpkg.json file.
pub struct RawNxpkgJSON {
...
const CONFIG_FILE: &str = "nxpkg.json";
...
impl RawNxpkgJSON {
    pub(crate) fn read(path: &AbsoluteSystemPath) -> Result<RawNxpkgJSON, Error> {
        let contents = path.read()?;
        let raw_nxpkg_json: RawNxpkgJSON =
            serde_json::from_reader(json_comments::StripComments::new(contents.as_slice()))?;

        Ok(raw_nxpkg_json)
    }

    /// Produces a new nxpkg.json without any tasks that reference non-existent
    /// workspaces
...
impl TryFrom<RawNxpkgJSON> for NxpkgJson {
    type Error = Error;

    fn try_from(raw_nxpkg: RawNxpkgJSON) -> Result<Self, Error> {
        let mut global_env = HashSet::new();
        let mut global_file_dependencies = HashSet::new();

        if let Some(global_env_from_nxpkg) = raw_nxpkg.global_env {
            gather_env_vars(global_env_from_nxpkg, "globalEnv", &mut global_env)?;
        }

        // TODO: In the rust port, warnings should be refactored to a post-parse
        // validation step
        for value in raw_nxpkg.global_dependencies.into_iter().flatten() {
        ...
        Ok(NxpkgJson {
            ...
            global_pass_through_env: raw_nxpkg
            ...
            remote_cache: raw_nxpkg.remote_cache,
            extends: raw_nxpkg.extends.unwrap_or_default(),
            // Directly to space_id, we don't need to keep the struct
            space_id: raw_nxpkg.experimental_spaces.and_then(|s| s.id),
        })
    }
}

impl NxpkgJson {
    /// Loads nxpkg.json by reading the file at `dir` and optionally combining
    ...
    pub fn load(
    ...
    ) -> Result<NxpkgJson, Error> {
        if root_package_json.legacy_nxpkg_config.is_some() {
            println!(
                "[WARNING] \"nxpkg\" in package.json is no longer supported. Migrate to {} by \
                 running \"npx @turbo/codemod create-nxpkg-config\"\n",
                CONFIG_FILE
            );
        }

        let nxpkg_from_files = Self::read(&dir.join_component(CONFIG_FILE));

        let mut nxpkg_json = match (include_synthesized_from_root_package_json, nxpkg_from_files) {
            // If the file didn't exist, throw a custom error here instead of propagating
            (false, Err(Error::Io(_))) => return Err(Error::NoNxpkgJSON),
            ...
            // tasks
            (true, Ok(mut nxpkg_from_files)) => {
                let mut pipeline = Pipeline::default();
                for (task_name, task_definition) in nxpkg_from_files.pipeline {
                ...
                nxpkg_from_files.pipeline = pipeline;
        ...
        for script_name in root_package_json.scripts.keys() {
            ...
            if !nxpkg_json.has_task(&task_name) {
    ...
    /// Reads a `RawNxpkgJson` from the given path
    /// and then converts it into `NxpkgJson`
    pub(crate) fn read(path: &AbsoluteSystemPath) -> Result<NxpkgJson, Error> {
        let raw_nxpkg_json = RawNxpkgJSON::read(path)?;
        raw_nxpkg_json.try_into()
    }
    ...
    pub fn validate(&self, validations: &[NxpkgJSONValidation]) -> Vec<Error> {
        validations
            .iter()
...
type NxpkgJSONValidation = fn(&NxpkgJson) -> Vec<Error>;

pub fn validate_no_package_task_syntax(nxpkg_json: &NxpkgJson) -> Vec<Error> {
    nxpkg_json
        .pipeline
        .keys()
...
pub fn validate_extends(nxpkg_json: &NxpkgJson) -> Vec<Error> {
    match nxpkg_json.extends.first() {
        Some(package_name) if package_name != ROOT_PKG_NAME || nxpkg_json.extends.len() > 1 => {
...
mod tests {
    ...
    use nxpkgpath::{AbsoluteSystemPath, RelativeUnixPathBuf};
    use nxpkgrepo_repository::package_json::PackageJson;

    use super::RawNxpkgJSON;
    use crate::{
        cli::OutputLogsMode,
        config::{nxpkg::RawTaskDefinition, NxpkgJson},
    ...
    #[test_case(r"{}", NxpkgJson::default() ; "empty")]
    ...
    fn test_get_root_nxpkg_no_synthesizing(
        nxpkg_json_content: &str,
        expected_nxpkg_json: NxpkgJson,
    ) -> Result<()> {
        let root_dir = tempdir()?;
        let root_package_json = PackageJson::default();
        let repo_root = AbsoluteSystemPath::from_std_path(root_dir.path())?;
        fs::write(repo_root.join_component("nxpkg.json"), nxpkg_json_content)?;

        let nxpkg_json = NxpkgJson::load(repo_root, &root_package_json, false)?;
        assert_eq!(nxpkg_json, expected_nxpkg_json);
    ...
    #[test_case(
        Some("{}"),
        PackageJson {
            legacy_nxpkg_config: Some(serde_json::Value::String("build".to_string())),
            ..PackageJson::default()
        },
        NxpkgJson::default()
    )]
    ...
    fn test_get_root_nxpkg_with_synthesizing(
        nxpkg_json_content: Option<&str>,
        root_package_json: PackageJson,
        expected_nxpkg_json: NxpkgJson,
    ) -> Result<()> {
        let root_dir = tempdir()?;
        ...
        if let Some(content) = nxpkg_json_content {
            fs::write(repo_root.join_component("nxpkg.json"), content)?;
        }

        let nxpkg_json = NxpkgJson::load(repo_root, &root_package_json, true)?;
        assert_eq!(nxpkg_json, expected_nxpkg_json);
    ...
    #[test]
    fn test_nxpkg_task_pruning() {
        let json: RawNxpkgJSON = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "//#top": {},
                "build": {},
                "a#build": {},
        ...
        let expected: RawNxpkgJSON = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "//#top": {},
                "build": {},
                "a#build": {},
    ...
    fn test_parsing_output_mode(output_mode: &str, expected: Option<OutputLogsMode>) {
        let json: Result<RawNxpkgJSON, _> = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "build": {
