...
/// A basic logger that logs to stderr using the NxpkgFormatter.
/// The first generic parameter refers to the previous layer, which
/// is in this case the default layer (`Registry`).
type StdErrLog = fmt::Layer<Registry, DefaultFields, NxpkgFormatter, StdErrWrapper>;
...
/// - with evel `TRACE` or higher for the `nxpkgrepo` target
...
impl NxpkgSubscriber {
    ...
    /// - If the `NXPKG_LOG_VERBOSITY` env var is set, it will be used to set
    ...
    ///   `NXPKG_LOG_VERBOSITY` global setting, but not per-module settings.
    ///
    /// `NxpkgSubscriber` has optional loggers that can be enabled later:
    ...
    pub fn new_with_verbosity(verbosity: usize, ui: &UI) -> Self {
        ...
        let env_filter = || {
            let filter = EnvFilter::builder()
                ...
                .with_env_var("NXPKG_LOG_VERBOSITY")
                .from_env_lossy();
        ...
        let daemon_filter = tracing_subscriber::filter::targets::Targets::new()
            ...
            .with_target("nxpkgrepo", Level::TRACE);

        let stderr = fmt::layer()
            .with_writer(StdErrWrapper {})
            .event_format(NxpkgFormatter::new_with_ansi(!ui.should_strip_ansi))
...
impl<S, N> FormatEvent<S, N> for NxpkgFormatter
where
