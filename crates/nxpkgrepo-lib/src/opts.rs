...
use thiserror::Error;
use nxpkgpath::AnchoredSystemPathBuf;
use nxpkgrepo_cache::CacheOpts;
...
impl<'a> Opts<'a> {
    pub fn synthesize_command(&self) -> String {
        let mut cmd = format!("nxpkg run {}", self.run_opts.tasks.join(" "));
...
impl<'a> TryFrom<&'a RunArgs> for RunOpts<'a> {
    ...
    fn try_from(args: &'a RunArgs) -> Result<Self, Self::Error> {
        ...
        let (is_github_actions, log_order, log_prefix) = match args.log_order {
            LogOrder::Auto if nxpkgrepo_ci::Vendor::get_constant() == Some("GITHUB_ACTIONS") => (
                true,
...
mod test {
    ...
    #[test_case(
        ...
        "nxpkg run build --filter=my-app -- -v --foo=bar"
    )]
    #[test_case(
        TestCaseOpts {
    ...
    #[test_case(
        ...
        "nxpkg run build --filter=other-app --filter=my-app -- -v --foo=bar"
    )]
    #[test_case    (
        TestCaseOpts {
            legacy_filter: Some(LegacyFilter {
        ...
        "nxpkg run build --filter=other-app --filter=...my-app...[some-ref]..."
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nxpkg run build --filter=my-app --parallel --continue"
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nxpkg run build --filter=my-app --dry"
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nxpkg run build --filter=my-app --dry=json"
    )]
