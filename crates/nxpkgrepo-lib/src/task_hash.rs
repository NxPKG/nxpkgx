...
use nxpkgrepo_cache::CacheHitMetadata;
use nxpkgrepo_env::{BySource, DetailedMap, EnvironmentVariableMap, ResolvedEnvMode};
use nxpkgrepo_repository::package_graph::{WorkspaceInfo, WorkspaceName};
use nxpkgrepo_scm::SCM;

use crate::{
    engine::TaskNode,
    framework::infer_framework,
    hash::{FileHashes, LockFilePackages, TaskHashable, NxpkgHash},
    opts::Opts,
    run::task_id::TaskId,
        ...
        let framework_slug = if do_framework_inference {
            ...
            if let Some(framework) = infer_framework(workspace, is_monorepo) {
                ...
                if let Some(exclude_prefix) =
                    self.env_at_execution_start.get("NXPKG_CI_VENDOR_ENV_KEY")
                {
...
pub fn get_external_deps_hash(
    transitive_dependencies: &Option<HashSet<nxpkgrepo_lockfiles::Package>>,
) -> String {
