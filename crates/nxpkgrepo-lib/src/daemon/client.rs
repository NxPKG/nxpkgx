...
use self::proto::nxpkgd_client::NxpkgdClient;
use super::{
...
pub mod proto {
    tonic::include_proto!("nxpkgdprotocol");
}

#[derive(Debug, Clone)]
pub struct DaemonClient<T: Clone> {
    client: NxpkgdClient<tonic::transport::Channel>,
    connect_settings: T,
}
...
impl DaemonClient<()> {
    pub fn new(client: NxpkgdClient<tonic::transport::Channel>) -> Self {
        Self {
            client,
            connect_settings: (),
        }
...
impl DaemonClient<DaemonConnector> {
    ...
    pub fn pid_file(&self) -> &nxpkgpath::AbsoluteSystemPathBuf {
        &self.connect_settings.pid_file
    }

    pub fn sock_file(&self) -> &nxpkgpath::AbsoluteSystemPathBuf {
...
pub enum DaemonError {
    ...
    /// The server is running a different version of nxpkgrepo.
...
mod test {
    ...
    fn test_format_repo_relative_glob() {
        let raw_glob = ["some", ".nxpkg", "nxpkg-foo:bar.log"].join(MAIN_SEPARATOR_STR);
        #[cfg(windows)]
        let expected = "some/.nxpkg/nxpkg-foo";
        #[cfg(not(windows))]
        let expected = "some/.nxpkg/nxpkg-foo\\:bar.log";
