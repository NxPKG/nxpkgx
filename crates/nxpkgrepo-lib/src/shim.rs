...
use tracing::debug;
use nxpkg_updater::check_for_updates;
use nxpkgpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf};
use nxpkgrepo_repository::{
    inference::{RepoMode, RepoState},
    package_json::PackageJson,
};
use nxpkgrepo_ui::UI;

use crate::{cli, get_version, spawn_child, tracing::NxpkgSubscriber, Payload};
...
pub enum Error {
    ...
    Cli(#[from] cli::Error),
    #[error(transparent)]
    Inference(#[from] nxpkgrepo_repository::inference::Error),
    #[error("failed to execute local nxpkg process")]
    LocalNxpkgProcess(#[source] std::io::Error),
    #[error("failed to resolve local nxpkg path: {0}")]
    LocalNxpkgPath(String),
...
// example)
static NXPKG_PURE_OUTPUT_ARGS: [&str; 6] = [
    "--json",
    "--dry",
    "--dry-run",
    "--dry=json",
...
static NXPKG_SKIP_NOTIFIER_ARGS: [&str; 5] =
    ["--help", "--h", "--version", "--v", "--no-update-notifier"];

fn nxpkg_version_has_shim(version: &str) -> bool {
...
struct ShimArgs {
    ...
    remaining_nxpkg_args: Vec<String>,
...
impl ShimArgs {
    pub fn parse() -> Result<Self, Error> {
        ...
        let mut remaining_nxpkg_args = Vec::new();
    ...
    fn has_json_flags(&self) -> bool {
        self.remaining_nxpkg_args
            .iter()
            .any(|arg| NXPKG_PURE_OUTPUT_ARGS.contains(&arg.as_str()))
    }
    ...
    fn has_notifier_skip_flags(&self) -> bool {
        self.remaining_nxpkg_args
            .iter()
            .any(|arg| NXPKG_SKIP_NOTIFIER_ARGS.contains(&arg.as_str()))
    }
...
#[derive(Debug)]
pub struct NxpkgState {
    bin_path: Option<PathBuf>,
    version: &'static str,
    repo_state: Option<RepoState>,
}

impl Default for NxpkgState {
    fn default() -> Self {
        Self {
...
impl NxpkgState {
    ...
    pub const fn platform_package_name() -> &'static str {
        formatcp!("nxpkg-{}", NxpkgState::platform_name())
    }
...
#[derive(Debug)]
pub struct LocalNxpkgState {
    bin_path: PathBuf,
    version: String,
}

impl LocalNxpkgState {
    // Hoisted strategy:
    // - `bun install`
    // - `npm install`
    // - `yarn`
    ...
    fn generate_nested_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        Some(root_path.join_components(&["node_modules", "nxpkg", "node_modules"]))
    }
    ...
    fn generate_unplugged_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        let platform_package_name = NxpkgState::platform_package_name();
    ...
    // We support six per-platform packages and one `nxpkg` package which handles
    ...
    // _worst_ outcome is that we run global `nxpkg`.
    //
    ...
    pub fn infer(root_path: &AbsoluteSystemPath) -> Option<Self> {
        let platform_package_name = NxpkgState::platform_package_name();
        let binary_name = NxpkgState::binary_name();
            ...
            .filter_map(|search_function| search_function(root_path))
        {
            ...
            match fs_canonicalize(&bin_path) {
                Ok(bin_path) => {
                    ...
                    debug!("Local nxpkg path: {}", bin_path.display());
                    debug!("Local nxpkg version: {}", &local_version);
                    return Some(Self {
                        bin_path,
                        version: local_version,
                    });
                }
                Err(_) => debug!("No local nxpkg binary found at: {}", bin_path),
            }
        }
    ...
    fn supports_skip_infer_and_single_package(&self) -> bool {
        nxpkg_version_has_shim(&self.version)
    }
...
/// Attempts to run correct nxpkg by finding nearest package.json,
/// then finding local nxpkg installation. If the current binary is the
/// local nxpkg installation, then we run current nxpkg. Otherwise we
/// kick over to the local nxpkg installation.
///
/// # Arguments
///
/// * `nxpkg_state`: state for current execution
///
/// returns: Result<i32, Error>
fn run_correct_nxpkg(
    repo_state: RepoState,
    shim_args: ShimArgs,
    subscriber: &NxpkgSubscriber,
    ui: UI,
) -> Result<Payload, Error> {
    if let Some(nxpkg_state) = LocalNxpkgState::infer(&repo_state.root) {
        try_check_for_updates(&shim_args, &nxpkg_state.version);

        if nxpkg_state.local_is_self() {
            env::set_var(
                cli::INVOCATION_DIR_ENV_VAR,
                shim_args.invocation_dir.as_path(),
            );
            debug!("Currently running nxpkg is local nxpkg.");
            Ok(cli::run(Some(repo_state), subscriber, ui)?)
        } else {
            Ok(Payload::Rust(spawn_local_nxpkg(
                &repo_state,
                nxpkg_state,
                shim_args,
            )))
        ...
        debug!("Running command as global nxpkg");
        Ok(cli::run(Some(repo_state), subscriber, ui)?)
    }
}

fn spawn_local_nxpkg(
    repo_state: &RepoState,
    local_nxpkg_state: LocalNxpkgState,
    mut shim_args: ShimArgs,
) -> Result<i32, Error> {
    let local_nxpkg_path = fs_canonicalize(&local_nxpkg_state.bin_path).map_err(|_| {
        Error::LocalNxpkgPath(local_nxpkg_state.bin_path.to_string_lossy().to_string())
    })?;
    debug!(
        "Running local nxpkg binary in {}\n",
        local_nxpkg_path.display()
    );

    let supports_skip_infer_and_single_package =
        local_nxpkg_state.supports_skip_infer_and_single_package();
    ...
    raw_args.append(&mut shim_args.remaining_nxpkg_args);
    ...
    // We spawn a process that executes the local nxpkg
    // that we've found in node_modules/.bin/nxpkg.
    let mut command = process::Command::new(local_nxpkg_path);
    command
        .args(&raw_args)
        // rather than passing an argument that local nxpkg might not understand, set
    ...
    let child = spawn_child(command).map_err(Error::LocalNxpkgProcess)?;

    let exit_status = child.wait().map_err(Error::LocalNxpkgProcess)?;
    let exit_code = exit_status.code().unwrap_or_else(|| {
        debug!("go-nxpkg failed to report exit code");
        #[cfg(unix)]
        {
            ...
            debug!(
                "go-nxpkg caught signal {:?}. Core dumped? {}",
                signal, core_dumped
            );
        }
        2
...
/// Checks for `NXPKG_BINARY_PATH` variable. If it is set,
/// we do not try to find local nxpkg, we simply run the command as
/// the current binary. This is due to legacy behavior of `NXPKG_BINARY_PATH`
/// that lets users dynamically set the path of the nxpkg binary. Because
/// that conflicts with finding a local nxpkg installation and
/// executing that binary, these two features are fundamentally incompatible.
fn is_nxpkg_binary_path_set() -> bool {
    env::var("NXPKG_BINARY_PATH").is_ok()
}
...
pub fn run() -> Result<Payload, Error> {
    ...
    let ui = args.ui();
    let subscriber = NxpkgSubscriber::new_with_verbosity(args.verbosity, &ui);

    debug!("Global nxpkg version: {}", get_version());

    // If skip_infer is passed, we're probably running local nxpkg with
    // global nxpkg having handled the inference. We can run without any
    // concerns.
    if args.skip_infer {
        return Ok(cli::run(None, &subscriber, ui)?);
    }

    // If the NXPKG_BINARY_PATH is set, we do inference but we do not use
    // it to execute local nxpkg. We simply use it to set the `--single-package`
    // and `--cwd` flags.
    if is_nxpkg_binary_path_set() {
    ...
    match RepoState::infer(&args.cwd) {
        Ok(repo_state) => {
            debug!("Repository Root: {}", repo_state.root);
            run_correct_nxpkg(repo_state, args, &subscriber, ui)
        }
        Err(err) => {
            // If we cannot infer, we still run global nxpkg. This allows for global
...
#[cfg(test)]
mod test {
    use super::nxpkg_version_has_shim;
    ...
    fn test_skip_infer_version_constraint() {
        ...
        assert!(nxpkg_version_has_shim(release));
        assert!(nxpkg_version_has_shim(canary));
        assert!(nxpkg_version_has_shim(newer_canary));
        assert!(nxpkg_version_has_shim(newer_minor_canary));
        assert!(nxpkg_version_has_shim(new));
        assert!(nxpkg_version_has_shim(new_major));
        assert!(!nxpkg_version_has_shim(old));
        assert!(!nxpkg_version_has_shim(old_canary));
    }
}
