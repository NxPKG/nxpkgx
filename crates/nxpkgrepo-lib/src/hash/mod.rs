//! hash module
//!
//! This module contains the hash functions used by nxpkgrepo for certain
...
pub use traits::NxpkgHash;
use nxpkgrepo_env::{EnvironmentVariablePairs, ResolvedEnvMode};

use crate::{cli::EnvMode, task_graph::TaskOutputs};

mod proto_capnp {
    use nxpkgrepo_env::ResolvedEnvMode;
...
pub struct TaskHashable<'a> {
    ...
    // task
    pub(crate) package_dir: Option<nxpkgpath::RelativeUnixPathBuf>,
    ...
    pub(crate) dot_env: &'a [nxpkgpath::RelativeUnixPathBuf],
}

#[derive(Debug, Clone)]
pub struct GlobalHashable<'a> {
    pub global_cache_key: &'static str,
    pub global_file_hash_map: &'a HashMap<nxpkgpath::RelativeUnixPathBuf, String>,
    ...
    pub dot_env: &'a [nxpkgpath::RelativeUnixPathBuf],
}

pub struct LockFilePackages(pub Vec<nxpkgrepo_lockfiles::Package>);

#[derive(Debug, Clone)]
pub struct FileHashes(pub HashMap<nxpkgpath::RelativeUnixPathBuf, String>);
...
impl From<LockFilePackages> for Builder<HeapAllocator> {
    fn from(LockFilePackages(packages): LockFilePackages) -> Self {
...

        {
            ...
            for (i, nxpkgrepo_lockfiles::Package { key, version }) in packages.iter().enumerate() {
...
mod test {
    ...
    use nxpkgrepo_env::ResolvedEnvMode;
    use nxpkgrepo_lockfiles::Package;

    use super::{
        FileHashes, GlobalHashable, LockFilePackages, TaskHashable, TaskOutputs, NxpkgHash,
    };
    use crate::cli::EnvMode;
    ...
    fn task_hashable() {
        let task_hashable = TaskHashable {
            ...
            package_dir: Some(nxpkgpath::RelativeUnixPathBuf::new("package_dir").unwrap()),
            ...
            dot_env: &[nxpkgpath::RelativeUnixPathBuf::new("dotenv".to_string()).unwrap()],
        };
    ...
    fn global_hashable() {
        let global_file_hash_map = vec![(
            nxpkgpath::RelativeUnixPathBuf::new("global_file_hash_map").unwrap(),
    ...
    fn file_hashes(pairs: Vec<(String, String)>, expected: &str) {
        let file_hashes = FileHashes(
            pairs
                .into_iter()
                .map(|(a, b)| (nxpkgpath::RelativeUnixPathBuf::new(a).unwrap(), b))
                .collect(),
        );
