...
pub async fn daemon_client(command: &DaemonCommand, base: &CommandBase) -> Result<(), DaemonError> {
    ...
    let pid_file = base.daemon_file_root().join_component("nxpkgd.pid");
    let sock_file = base.daemon_file_root().join_component("nxpkgd.sock");
...
pub async fn daemon_server(
    base: &CommandBase,
    idle_time: &String,
    logging: &NxpkgSubscriber,
) -> Result<(), DaemonError> {
    let (log_folder, log_file) = {
        let directories = directories::ProjectDirs::from("com", "nxpkgrepo", "nxpkgrepo")
        ...
        let log_file =
            log_folder.join_component(format!("{}-nxpkg.log", base.repo_hash()).as_str());
