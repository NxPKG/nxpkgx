...
use nxpkgrepo_ui::BOLD;

use super::CommandBase;
use crate::config::RawNxpkgJSON;
...
pub enum Error {
    ...
    #[error(transparent)]
    Lockfile(#[from] nxpkgrepo_lockfiles::Error),
    #[error("nxpkg doesn't support workspaces at file system root")]
...
fn nxpkg_json() -> &'static AnchoredSystemPath {
    static PATH: OnceLock<&'static AnchoredSystemPath> = OnceLock::new();
    PATH.get_or_init(|| AnchoredSystemPath::new("nxpkg.json").unwrap())
}
...
pub fn prune(
    ...
    if matches!(
        ...
        nxpkgrepo_repository::package_manager::PackageManager::Bun
    ) {
    ...
    prune.copy_nxpkg_json(&workspace_names)?;
...
impl<'a> Prune<'a> {
    ...
    fn copy_file(
            ...
            )
        {
            ...
            nxpkgrepo_fs::copy_file(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_directory(
        ...
        let full_to = self.full_directory.resolve(path);
        nxpkgrepo_fs::recursive_copy(&from_path, full_to)?;
        if matches!(destination, Some(CopyDestination::All)) {
            let out_to = self.out_directory.resolve(path);
            nxpkgrepo_fs::recursive_copy(&from_path, out_to)?;
        }
        if self.docker
            && matches!(
                destination,
        ...
        {
            let docker_to = self.docker_directory().resolve(path);
            nxpkgrepo_fs::recursive_copy(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_workspace(&self, package_json_path: &AnchoredSystemPath) -> Result<(), Error> {
        ...
        nxpkgrepo_fs::recursive_copy(original_dir, &target_dir)?;
    ...
    fn copy_nxpkg_json(&self, workspaces: &[String]) -> Result<(), Error> {
        let original_nxpkg_path = self.root.resolve(nxpkg_json());

        let new_nxpkg_path = self.full_directory.resolve(nxpkg_json());

        let nxpkg_json_contents = match original_nxpkg_path.read() {
            Ok(contents) => contents,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                // If nxpkg.json doesn't exist skip copying
                return Ok(());
            }
            Err(e) => return Err(e.into()),
        };
        let nxpkg_json: RawNxpkgJSON = serde_json::from_reader(json_comments::StripComments::new(
            nxpkg_json_contents.as_slice(),
        ))?;

        let pruned_nxpkg_json = nxpkg_json.prune_tasks(workspaces);
        new_nxpkg_path.create_with_contents(serde_json::to_string_pretty(&pruned_nxpkg_json)?)?;
