...
use rand::Rng;
use thiserror::Error;
use nxpkgrepo_api_client::Client;
#[cfg(not(test))]
use nxpkgrepo_ui::CYAN;
use nxpkgrepo_ui::{BOLD, GREY, UNDERLINE};
use nxpkgrepo_vercel_api::{CachingStatus, Space, Team};
...
pub enum Error {
    ...
    #[error("User not found. Please login to Nxpkgrepo first by running {command}.")]
    ...
    EnableCaching,
    #[error(
        "Could not persist selected space ({space_id}) to `experimentalSpaces.id` in nxpkg.json"
    )]
    WriteToNxpkgJson {
        space_id: String,
        #[source]
...
pub(crate) const REMOTE_CACHING_INFO: &str = "  Remote Caching shares your cached Nxpkgrepo task \
...
pub(crate) const REMOTE_CACHING_URL: &str =
    "https://nxpkg.build/repo/docs/core-concepts/remote-caching";
...
pub async fn link(
    ...
    let token = base.config()?.token().ok_or_else(|| Error::TokenNotFound {
        command: base.ui.apply(BOLD.apply_to("`npx nxpkg login`")),
    })?;
    ...
    {}
      ...
      ",
            ...
            if modify_gitignore {
                add_nxpkg_to_gitignore(base).map_err(|error| config::Error::FailedToSetConfig {
    ...
    {}
        ",
                ...
                GREY.apply_to("To disable Remote Caching, run `npx nxpkg unlink`")
            );
            Ok(())
        }
        LinkTarget::Spaces => {
      ...
      ",
            ...
            add_space_id_to_nxpkg_json(base, &space.id)?;
    ...
    {}
        ",
                ...
                GREY.apply_to(
                    "To remove Spaces integration, run `npx nxpkg unlink --target spaces`"
                )
            );
...
fn select_team<'a>(
    ...
    let prompt = format!(
        ...
        base.ui.apply(BOLD.apply_to(
            "Which Vercel scope (and Remote Cache) do you want associated with this Nxpkgrepo?",
        )),
        base.ui
...
fn select_space<'a>(base: &CommandBase, spaces: &'a [Space]) -> Result<SelectedSpace<'a>, Error> {
    ...
    let prompt = format!(
        "{}\n  {}",
        base.ui.apply(
            BOLD.apply_to("Which Vercel space do you want associated with this Nxpkgrepo?",)
        ),
        base.ui
            .apply(CYAN.apply_to("[Use arrows to move, type to filter]"))
    );
...
fn add_nxpkg_to_gitignore(base: &CommandBase) -> Result<(), io::Error> {
    ...
    if !gitignore_path.exists() {
        ...
        writeln!(gitignore, ".nxpkg")?;
    } else {
        let gitignore = File::open(&gitignore_path)?;
        let mut lines = io::BufReader::new(gitignore).lines();
        let has_nxpkg = lines.any(|line| line.map_or(false, |line| line.trim() == ".nxpkg"));
        if !has_nxpkg {
...
fn add_space_id_to_nxpkg_json(base: &CommandBase, space_id: &str) -> Result<(), Error> {
    let nxpkg_json_path = base.repo_root.join_component("nxpkg.json");
    let nxpkg_json = nxpkg_json_path
        .read_existing_to_string_or(Ok("{}"))
        .map_err(|error| config::Error::FailedToReadConfig {
    ...
    fs::write(nxpkg_json_path, output).map_err(|error| Error::WriteToNxpkgJson {
...
mod test {
    ...
    use nxpkgrepo_ui::UI;
    use nxpkgrepo_vercel_api_mock::start_test_server;

    use crate::{
        cli::LinkTarget,
        commands::{link, CommandBase},
        config::{RawNxpkgJSON, NxpkgrepoConfigBuilder},
        Args,
    };
    ...
    async fn test_link_remote_cache() -> Result<()> {
        ...
        let repo_config_path = repo_root.join_components(&[".nxpkg", "config.json"]);
        ...
        base.config
            .set(
                NxpkgrepoConfigBuilder::new(&base)
        ...
        // read the config
        let updated_config = NxpkgrepoConfigBuilder::new(&base).build().unwrap();
        let team_id = updated_config.team_id();

        assert!(
            team_id == Some(nxpkgrepo_vercel_api_mock::EXPECTED_USER_ID)
                || team_id == Some(nxpkgrepo_vercel_api_mock::EXPECTED_TEAM_ID)
        );
    ...
    async fn test_link_spaces() {
        ...
        // nxpkg config
        let nxpkg_json_file = base.repo_root.join_component("nxpkg.json");

        fs::write(
            nxpkg_json_file.as_path(),
        ...
        // verify space id is added to nxpkg.json
        let nxpkg_json_contents = fs::read_to_string(&nxpkg_json_file).unwrap();
        let nxpkg_json: RawNxpkgJSON = serde_json::from_str(&nxpkg_json_contents).unwrap();
        assert_eq!(
            nxpkg_json.experimental_spaces.unwrap().id.unwrap(),
            nxpkgrepo_vercel_api_mock::EXPECTED_SPACE_ID
        );
    }
}
