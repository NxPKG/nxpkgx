...
use nxpkgrepo_graph_utils as graph;
...
use super::Engine;
use crate::{
    config::{validate_extends, validate_no_package_task_syntax, NxpkgJson},
...
pub enum Error {
    ...
    #[error(
        "{task_id} needs an entry in nxpkg.json before it can be depended on because it is a task \
    ...
    #[error("Could not find \"{task_id}\" in root nxpkg.json or \"{task_name}\" in workspace")]
    MissingWorkspaceTask { task_id: String, task_name: String },
    #[error(transparent)]
    Config(#[from] crate::config::Error),
    #[error("Invalid nxpkg.json:\n{error_lines}")]
...
pub struct EngineBuilder<'a> {
    ...
    is_single: bool,
    nxpkg_jsons: Option<HashMap<WorkspaceName, NxpkgJson>>,
...
impl<'a> EngineBuilder<'a> {
    ...
    pub fn with_nxpkg_jsons(
        mut self,
        nxpkg_jsons: Option<HashMap<WorkspaceName, NxpkgJson>>,
    ) -> Self {
        self.nxpkg_jsons = nxpkg_jsons;
        self
    }
    ...
    pub fn build(mut self) -> Result<super::Engine, Error> {
        ...
        let mut nxpkg_jsons = self.nxpkg_jsons.take().unwrap_or_default();
        ...
        for (workspace, task) in self.workspaces.iter().cartesian_product(self.tasks.iter()) {
            ...
            if self.has_task_definition(&mut nxpkg_jsons, workspace, task, &task_id)? {
    ...
    fn has_task_definition(
        &self,
        nxpkg_jsons: &mut HashMap<WorkspaceName, NxpkgJson>,
        workspace: &WorkspaceName,
        task_name: &TaskName<'static>,
        task_id: &TaskId,
    ) -> Result<bool, Error> {
        let nxpkg_json = self
            .nxpkg_json(nxpkg_jsons, workspace)
            // If there was no nxpkg.json in the workspace, fallback to the root nxpkg.json
            .or_else(|e| {
                if e.is_missing_nxpkg_json() && !matches!(workspace, WorkspaceName::Root) {
                    Ok(None)
                } else {
                    Err(e)
                }
        ...
        let Some(nxpkg_json) = nxpkg_json else {
            return self.has_task_definition(nxpkg_jsons, &WorkspaceName::Root, task_name, task_id);
        };

        let task_id_as_name = task_id.as_task_name();
        if nxpkg_json.pipeline.contains_key(&task_id_as_name)
            || nxpkg_json.pipeline.contains_key(task_name)
        {
            Ok(true)
        } else if !matches!(workspace, WorkspaceName::Root) {
            self.has_task_definition(nxpkg_jsons, &WorkspaceName::Root, task_name, task_id)
        } else {
            Ok(false)
        }
    }
    ...
    fn task_definition_chain(
        ...
        let root_nxpkg_json = self
            .nxpkg_json(nxpkg_jsons, &WorkspaceName::Root)?
            .ok_or(Error::Config(crate::config::Error::NoNxpkgJSON))?;

        if let Some(root_definition) = root_nxpkg_json.task(task_id, task_name) {
        ...
        if task_id.package() != ROOT_PKG_NAME {
            match self.nxpkg_json(nxpkg_jsons, &WorkspaceName::from(task_id.package())) {
                ...
                Err(e) if e.is_missing_nxpkg_json() => (),
                Err(e) => {
                    return Err(e);
                }
            }
    ...
    fn nxpkg_json<'b>(
        &self,
        nxpkg_jsons: &'b mut HashMap<WorkspaceName, NxpkgJson>,
        workspace: &WorkspaceName,
    ) -> Result<Option<&'b NxpkgJson>, Error> {
        if nxpkg_jsons.get(workspace).is_none() {
            let json = self.load_nxpkg_json(workspace)?;
            nxpkg_jsons.insert(workspace.clone(), json);
        }
        Ok(nxpkg_jsons.get(workspace))
    }

    fn load_nxpkg_json(&self, workspace: &WorkspaceName) -> Result<NxpkgJson, Error> {
...
impl Error {
    fn is_missing_nxpkg_json(&self) -> bool {
        matches!(self, Self::Config(crate::config::Error::NoNxpkgJSON))
    }
}
...
mod test {
    ...
    use nxpkgrepo_lockfiles::Lockfile;
    use nxpkgrepo_repository::{package_json::PackageJson, package_manager::PackageManager};

    use super::*;
    use crate::{config::RawNxpkgJSON, engine::TaskNode};
    ...
    impl Lockfile for MockLockfile {
        fn resolve_package(
            ...
            _name: &str,
            _version: &str,
        ) -> Result<Option<nxpkgrepo_lockfiles::Package>, nxpkgrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn all_dependencies(
            &self,
            _key: &str,
        ) -> Result<Option<HashMap<String, String>>, nxpkgrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> Result<Box<dyn Lockfile>, nxpkgrepo_lockfiles::Error> {
            unreachable!()
        }

        fn encode(&self) -> Result<Vec<u8>, nxpkgrepo_lockfiles::Error> {
            unreachable!()
        }
    ...
    fn test_nxpkg_json_loading() {
        ...
        let engine_builder = EngineBuilder::new(&repo_root, &package_graph, false)
            .with_nxpkg_jsons(Some(vec![].into_iter().collect()));

        let a_nxpkg_json = repo_root.join_components(&["packages", "a", "nxpkg.json"]);
        a_nxpkg_json.ensure_dir().unwrap();

        let result = engine_builder.load_nxpkg_json(&WorkspaceName::from("a"));
        assert!(
            result.is_err() && result.unwrap_err().is_missing_nxpkg_json(),
            "expected parsing to fail with missing nxpkg.json"
        );
        ...
        let nxpkg_json = engine_builder
            .load_nxpkg_json(&WorkspaceName::from("a"))
            .unwrap();
        assert_eq!(nxpkg_json.pipeline.len(), 1);
    }

    fn nxpkg_json(value: serde_json::Value) -> NxpkgJson {
        let raw: RawNxpkgJSON = serde_json::from_value(value).unwrap();
        NxpkgJson::try_from(raw).unwrap()
    }
    ...
    fn test_task_definition(
        ...
        let has_def = engine_builder
            .has_task_definition(&mut nxpkg_jsons, &workspace, &task_name, &task_id)
            .unwrap();
    ...
    fn test_default_engine() {
        ...
        let engine = EngineBuilder::new(&repo_root, &package_graph, false)
            .with_nxpkg_jsons(Some(nxpkg_jsons))
