...
use nxpkgrepo_repository::inference::{RepoMode, RepoState};
use nxpkgrepo_ui::UI;

use crate::{
    commands::{bin, daemon, generate, info, link, login, logout, prune, unlink, CommandBase},
    get_version,
    tracing::NxpkgSubscriber,
    Payload,
};

mod error;

// Global nxpkg sets this environment variable to its cwd so that local
// nxpkg can use it for package inference.
pub const INVOCATION_DIR_ENV_VAR: &str = "NXPKG_INVOCATION_DIR";
...
pub struct Args {
    ...
    #[clap(long, global = true)]
    #[serde(skip)]
    /// Skip any attempts to infer which version of Nxpkg the project is
    /// configured to use
    pub skip_infer: bool,
    /// Disable the nxpkg update notification
    #[clap(long, global = true)]
    #[serde(skip)]
    ...
    /// The directory in which to run nxpkg
    #[clap(long, global = true, value_parser)]
    ...
    #[clap(long, global = true)]
    pub no_color: bool,
    /// When enabled, nxpkg will precede HTTP requests with an OPTIONS request
    /// for authorization
    #[clap(long, global = true)]
    pub preflight: bool,
    ...
    /// Force a check for a new version of nxpkg
...
pub enum DaemonCommand {
    /// Restarts the nxpkg daemon
    Restart,
    /// Ensures that the nxpkg daemon is running
    Start,
    /// Reports the status of the nxpkg daemon
    Status {
        /// Pass --json to report status in JSON format
        #[clap(long)]
        json: bool,
    },
    /// Stops the nxpkg daemon
    Stop,
    /// Stops the nxpkg daemon if it is already running, and removes any stale
    /// daemon state
    Clean,
}
...
pub enum Command {
    ...
    /// Get the path to the Nxpkg binary
    Bin {},
    /// Generate the autocompletion script for the specified shell
    #[serde(skip)]
    Completion { shell: Shell },
    /// Runs the Nxpkgrepo background daemon
    Daemon {
        /// Set the idle timeout for nxpkgd
    ...
    Info {
        ...
        // We output nxpkg info as json. Currently just for internal testing
        #[clap(long)]
        json: bool,
    },
    ...
    ///
    /// By default, nxpkg executes tasks in topological order (i.e.
...
pub struct GenerateWorkspaceArgs {
    ...
    /// The root of your repository (default: directory with root nxpkg.json)
    #[clap(short = 'r', long)]
...
pub struct RunArgs {
    ...
    /// Run nxpkg in single-package mode
    #[clap(long)]
    pub single_package: bool,
    /// Use the given selector to specify package(s) to act as
    /// entry points. The syntax mirrors pnpm's syntax, and
    /// additional documentation and examples can be found in
    /// nxpkg's documentation https://nxpkg.build/repo/docs/reference/command-line-reference/run#--filter
    #[clap(short = 'F', long, action = ArgAction::Append)]
    pub filter: Vec<String>,
    /// Ignore the existing cache (to force execution)
    #[clap(long, env = "NXPKG_FORCE", default_missing_value = "true")]
    ...
    /// Use "strict" to use an allowlist specified in nxpkg.json.
    /// Use "infer" to defer to existence of "passThroughEnv" or
    /// "globalPassThroughEnv" in nxpkg.json. (default infer)
    ...
    #[clap(long)]
    pub no_cache: bool,
    /// Run without using nxpkg's daemon process
    #[clap(long)]
    pub no_daemon: bool,
    ...
    /// all output. Use "hash-only" to show only nxpkg-computed
    ...
    /// nxpkg decide based on its own heuristics. (default auto)
    #[clap(long, env = "NXPKG_LOG_ORDER", value_enum, default_value_t = LogOrder::Auto)]
    ...
    /// File to write nxpkg's performance profile output into.
    ...
    #[clap(long, env = "NXPKG_REMOTE_ONLY", value_name = "BOOL", action = ArgAction::Set, default_value = "false", default_missing_value = "true", num_args = 0..=1)]
    ...
    /// Generate a summary of the nxpkg run
    #[clap(long, env = "NXPKG_RUN_SUMMARY", default_missing_value = "true")]
    pub summarize: Option<Option<bool>>,

    /// Use "none" to remove prefixes from task logs. Use "task" to get task id
    /// prefixing. Use "auto" to let nxpkg decide how to prefix the logs
    ...
    // > Usage: nxpkg [OPTIONS] [TASKS]... [-- <FORWARDED_ARGS>...] [COMMAND]
    #[clap(hide = true)]
...
///
/// Scenarios:
/// 1. inference failed, we're running this global nxpkg. no repo state
/// 2. --skip-infer was passed, assume we're local nxpkg and run. no repo state
/// 3. There is no local nxpkg, we're running the global one. repo state exists
/// 4. nxpkg binary path is set, and it's this one. repo state exists
///
/// # Arguments
///
/// * `repo_state`: If we have done repository inference and NOT executed
/// local nxpkg, such as in the case where `NXPKG_BINARY_PATH` is set,
...
pub async fn run(
    ...
    #[allow(unused_variables)] logger: &NxpkgSubscriber,
    ui: UI,
    ...
    if let Command::Run(run_args) = &mut command {
        ...
        if cli_args.cwd.is_none() {
            if let Ok(invocation_dir) = env::var(INVOCATION_DIR_ENV_VAR) {
                ...
                // If repo state doesn't exist, we're either local nxpkg running at the root
                // (cwd), or inference failed.
                // If repo state does exist, we're global nxpkg, and want to calculate
    ...
    match cli_args.command.as_ref().unwrap() {
        ...
        Command::Run(args) => {
            ...
            // to the rust codepath for running nxpkg
        ...
        Command::Completion { shell } => {
            generate(*shell, &mut Args::command(), "nxpkg", &mut io::stdout());
...
mod test {
    ...
    fn test_parse_run() -> Result<()> {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--framework-inference"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    framework_inference: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--framework-inference", "true"])
                .unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--framework-inference", "false"])
                .unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--env-mode"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    env_mode: EnvMode::Infer,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--env-mode", "infer"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    env_mode: EnvMode::Infer,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--env-mode", "loose"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--env-mode", "strict"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "lint", "test"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--cache-dir", "foobar"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--cache-workers", "100"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    cache_workers: 100,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--concurrency", "20"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--continue"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--dry-run"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--dry-run", "json"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from([
                "nxpkg", "run", "build", "--filter", "water", "--filter", "earth", "--filter",
                "fire", "--filter", "air"
            ])
            .unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from([
                "nxpkg", "run", "build", "-F", "water", "-F", "earth", "-F", "fire", "-F", "air"
            ])
            .unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
        ...
        assert_eq!(
            Args::try_parse_from([
                "nxpkg", "run", "build", "--filter", "water", "-F", "earth", "--filter", "fire",
                "-F", "air"
            ])
            .unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--force"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--global-deps", ".env"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--graph"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--graph", "out.html"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--ignore", "foo.js"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from([
                "nxpkg", "run", "build", "--ignore", "foo.js", "--ignore", "bar.js"
            ])
            .unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--include-dependencies"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--no-cache"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    no_cache: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--no-daemon"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    no_daemon: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--no-deps"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    no_deps: true,
        ...
        assert_eq!(
            serde_json::to_string(&Args::try_parse_from(["nxpkg", "run", "build"]).unwrap())?
                .contains("\"output_logs\":null"),
            true
        );

        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--output-logs", "full"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--output-logs", "none"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--output-logs", "hash-only"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--log-order", "stream"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--log-order", "grouped"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--log-prefix", "auto"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--log-prefix", "none"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--log-prefix", "task"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--parallel"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    parallel: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--profile", "profile_out"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--remote-only"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    remote_only: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--remote-only", "true"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    remote_only: true,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--remote-only", "false"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
                    remote_only: false,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--scope", "foo", "--scope", "bar"])
                .unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--since", "foo"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "build"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "build", "lint", "test"]).unwrap(),
            Args {
                run_args: Some(RunArgs {
                    tasks: vec!["build".to_string(), "lint".to_string(), "test".to_string()],
                    ..get_default_run_args()
    ...
    fn test_parse_bin() {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "bin"]).unwrap(),
            Args {
    ...
    fn test_parse_login() {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "login"]).unwrap(),
            Args {
    ...
    fn test_parse_logout() {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "logout"]).unwrap(),
            Args {
    ...
    fn test_parse_unlink() {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "unlink"]).unwrap(),
            Args {
    ...
    fn test_parse_prune() {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "prune", "foo"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "prune", "--scope", "bar"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "prune", "foo", "bar"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "prune", "--docker", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
        ...
        assert_eq!(
            Args::try_parse_from(["nxpkg", "prune", "--out-dir", "dist", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
    ...
    fn test_pass_through_args() {
        assert_eq!(
            Args::try_parse_from(["nxpkg", "run", "build", "--", "--script-arg=42"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
    ...
    fn test_parse_prune_no_mixed_arg_and_flag() {
        assert!(Args::try_parse_from(["nxpkg", "prune", "foo", "--scope", "bar"]).is_err(),);
    }
