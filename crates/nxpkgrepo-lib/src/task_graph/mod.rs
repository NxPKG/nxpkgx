...
use nxpkgpath::{AnchoredSystemPath, AnchoredSystemPathBuf, RelativeUnixPathBuf};
...
fn task_log_filename(task_name: &str) -> String {
    format!("nxpkg-{}.log", task_name.replace(':', "$colon$"))
}
...
mod test {
        ...
        assert_eq!(
            ...
            TaskOutputs {
                inclusions: vec![
                    ...
                    format!("{relative_prefix}.nxpkg/nxpkg-build.log"),
                ],
    ...
    fn test_escape_log_file() {
        ...
        let build_expected =
            AnchoredSystemPathBuf::from_raw([".nxpkg", "nxpkg-build.log"].join(MAIN_SEPARATOR_STR))
                .unwrap();
        ...
        let build_expected = AnchoredSystemPathBuf::from_raw(
            [".nxpkg", "nxpkg-build$colon$prod.log"].join(MAIN_SEPARATOR_STR),
        )
        .unwrap();
        assert_eq!(build_log, build_expected);
        ...
        let build_expected = AnchoredSystemPathBuf::from_raw(
            [".nxpkg", "nxpkg-build$colon$prod$colon$extra.log"].join(MAIN_SEPARATOR_STR),
        )
        .unwrap();
        assert_eq!(build_log, build_expected);
    }
