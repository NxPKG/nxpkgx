...
use nxpkgrepo_ci::github_header_footer;
use nxpkgrepo_env::{EnvironmentVariableMap, ResolvedEnvMode};
use nxpkgrepo_repository::{
    package_graph::{PackageGraph, WorkspaceName, ROOT_PKG_NAME},
    package_manager::PackageManager,
};
use nxpkgrepo_ui::{ColorSelector, OutputClient, OutputSink, OutputWriter, PrefixedUI, UI};
use which::which;
...
pub enum Error {
    ...
    #[error(
        "root task {task_name} ({command}) looks like it invokes nxpkg and might cause a loop"
    )]
    RecursiveNxpkg { task_name: String, command: String },
...
impl<'a> Visitor<'a> {
    ...
    pub async fn visit(&self, engine: Arc<Engine>) -> Result<Vec<TaskError>, Error> {
        ...
        while let Some(message) = node_stream.recv().await {
            ...
            match command {
                Some(cmd) if info.package() == ROOT_PKG_NAME && nxpkg_regex().is_match(&cmd) => {
                    return Err(Error::RecursiveNxpkg {
            ...
            match self.dry {
                ...
                false => {
                    // TODO(gsoltis): if/when we fix https://github.com/nxpkg/nxpkg/issues/937
    ...
    fn output_client(&self, task_id: &TaskId) -> OutputClient<impl std::io::Write> {
        let behavior = match self.opts.run_opts.log_order {
            crate::opts::ResolvedLogOrder::Stream if self.run_tracker.spaces_enabled() => {
                nxpkgrepo_ui::OutputClientBehavior::InMemoryBuffer
            }
            crate::opts::ResolvedLogOrder::Stream => {
                nxpkgrepo_ui::OutputClientBehavior::Passthrough
            }
            crate::opts::ResolvedLogOrder::Grouped => nxpkgrepo_ui::OutputClientBehavior::Grouped,
        };
...
fn nxpkg_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?:^|\s)nxpkg(?:$|\s)").unwrap())
}
...
impl ExecContext {
    ...
    async fn execute_inner(
        ...
        cmd.env("NXPKG_HASH", &self.task_hash);
        ...
        let mut process = match self.manager.spawn(cmd, Duration::from_millis(500)) {
            Some(Ok(child)) => child,
            // Nxpkg was unable to spawn a process
            Some(Err(e)) => {
