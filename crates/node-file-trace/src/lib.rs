...
use nxpkg_tasks::{
    backend::Backend, util::FormatDuration, TaskId, TransientInstance, TransientValue, NxpkgTasks,
    NxpkgTasksBackendApi, UpdateInfo, Value, Vc,
};
use nxpkg_tasks_fs::{
...
use nxpkgpack_cli_utils::issue::{ConsoleUi, IssueSeverityCliOption, LogOptions};
use nxpkgpack_core::{
    ...
    for entry in result.results.values() {
        if let DirectoryEntry::File(path) = entry {
            ...
            list.push(asset_context.process(
                source,
                Value::new(nxpkgpack_core::reference_type::ReferenceType::Undefined),
            ));
        }
    }
...
pub async fn start(
    args: Arc<Args>,
    nxpkg_tasks: Option<&Arc<NxpkgTasks<MemoryBackend>>>,
    ...
    if let Some(cache) = cache {
        ...
        use nxpkg_tasks_memory::MemoryBackendWithPersistedGraph;
        use nxpkg_tasks_rocksdb::RocksDbPersistedGraph;
    ...
    run(
        args.clone(),
        || {
            nxpkg_tasks.cloned().unwrap_or_else(|| {
                NxpkgTasks::new(MemoryBackend::new(memory_limit.unwrap_or(usize::MAX)))
            })
        },
...
async fn run<B: Backend + 'static, F: Future<Output = ()>>(
    args: Arc<Args>,
    create_tt: impl Fn() -> Arc<NxpkgTasks<B>>,
    final_finish: impl FnOnce(Arc<NxpkgTasks<B>>, TaskId, Duration) -> F,
    module_options: Option<ModuleOptionsContext>,
    resolve_options: Option<ResolveOptionsContext>,
) -> Result<Vec<String>> {
    let &CommonArgs {
    ...
    let start = Instant::now();
    let finish = |tt: Arc<NxpkgTasks<B>>, root_task: TaskId| async move {
        if watch {
