...
use nxpkg_tasks::{primitives::Regex, Value, ValueToString, Vc};
use nxpkg_tasks_fs::{DirectoryContent, DirectoryEntry, FileSystemPath};
use nxpkgpack_core::{
...
/// wrapped in `__nxpkgpack_require_context__`;
#[nxpkg_tasks::value]
#[derive(Hash, Debug)]
    ...
    async fn code_generation(
        ...
        visitors.push(create_visitor!(path, visit_mut_expr(expr: &mut Expr) {
            if let Expr::Call(_) = expr {
                *expr = quote!(
                    "__nxpkgpack_require_context__(__nxpkgpack_require__($id))" as Expr,
...
impl EcmascriptChunkItem for RequireContextChunkItem {
    ...
    async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
        ...
        let expr = quote_expr!(
            "__nxpkgpack_export_value__($obj);",
