            ...
            .map(|element| async move {
                Ok(match element {
                    ...
                    AmdDefineDependencyElement::Require => {
                        ResolvedElement::Expr(quote!("__nxpkgpack_require__" as Expr))
                    }
                })
            })
            .try_join()
...
///   factory(
///     __nxpkgpack_require__(dep1),
///     __nxpkgpack_require__(dep2),
///   ),
/// );
/// ```
fn transform_amd_factory(
            ...
            ResolvedElement::PatternMapping(pm, req) => match &**pm {
                ...
                pm => {
                    ...
                    if pm.is_internal_import() {
                        quote_expr!("__nxpkgpack_require__($arg)", arg: Expr = arg)
                    } else {
                        quote_expr!("__nxpkgpack_external_require__($arg)", arg: Expr = arg)
                    }
                }
            },
    ...
    match factory_type {
        ...
        AmdDefineFactoryType::Function => {
            // (r => r !== undefined && __nxpkgpack_export_value__(r))(...([...]))
            *callee = Callee::Expr(quote_expr!(
                "r => r !== undefined && __nxpkgpack_export_value__(r)"
            ));
            args.push(ExprOrSpread {
        ...
        AmdDefineFactoryType::Value => {
            // __nxpkgpack_export_value__(...)
            *callee = Callee::Expr(quote_expr!("__nxpkgpack_export_value__"));
            args.push(ExprOrSpread {
                expr: factory,
                spread: None,
            });
