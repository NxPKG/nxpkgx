...
use nxpkg_tasks::{TryJoinIterExt, Upcast, Value, Vc};
use nxpkg_tasks_fs::{FileJsonContent, FileSystemPath};
use nxpkgpack_core::{
...
use nxpkgpack_swc_utils::emitter::IssueEmitter;
...
pub(crate) async fn analyze_ecmascript_module(
        ...
        comments.iter().for_each(|comment| match comment.kind {
            CommentKind::Line => {
                ...
                if let Some(m) = SOURCE_MAP_FILE_REFERENCE.captures(&comment.text) {
                    ...
                    // The reference is needed too for nxpkgtrace
    ...
    for r in import_references.iter_mut() {
        ...
        // passing that to other nxpkg tasks functions later.
        *r = r.resolve().await?;
    }
        ...
        match effect {
            ...
            Effect::ImportedBinding {
                ...
                if let Some(r) = import_references.get(esm_reference_index) {
                    if let Some("__nxpkgpack_module_id__") = export.as_deref() {
                        analysis
...
pub struct AstPath(#[nxpkg_tasks(trace_ignore)] Vec<AstParentKind>);

pub static NXPKGPACK_HELPER: &str = "__nxpkgpackHelper";

pub fn is_nxpkgpack_helper_import(import: &ImportDecl) -> bool {
    import.with.as_ref().map_or(false, |asserts| {
        asserts.props.iter().any(|assert| {
            assert
                ...
                .map_or(false, |ident| &*ident.sym == NXPKGPACK_HELPER)
        })
    })
}
...
fn detect_dynamic_export(p: &Program) -> DetectedDynamicExportType {
    ...
    if let Program::Module(m) = p {
        ...
        if m.body.iter().any(|item| {
            item.as_module_decl().map_or(false, |module_decl| {
                module_decl
                    .as_import()
                    .map_or(true, |import| !is_nxpkgpack_helper_import(import))
            })
        }) {
    ...
    impl Visit for Visitor {
        ...
        fn visit_ident(&mut self, i: &Ident) {
            ...
            if &*i.sym == "__nxpkgpack_export_value__" {
                self.value = true;
                self.found = true;
            }
            if &*i.sym == "__nxpkgpack_export_namespace__" {
                self.namespace = true;
                self.found = true;
            }
        }
