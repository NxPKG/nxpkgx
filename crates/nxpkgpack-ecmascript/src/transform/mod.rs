...
use nxpkg_tasks::{ValueDefault, Vc};
...
/// the nxpkg_task caching requirements.
#[nxpkg_tasks::value(
    transparent,
    serialization = "none",
    eq = "manual",
...
#[derive(Debug)]
pub struct TransformPlugin(#[nxpkg_tasks(trace_ignore)] Box<dyn CustomTransformer + Send + Sync>);
                ...
                let config = Options {
                    ...
                    refresh: if *refresh {
                        Some(swc_core::ecma::transforms::react::RefreshOptions {
                            refresh_reg: "__nxpkgpack_refresh__.register".to_string(),
                            refresh_sig: "__nxpkgpack_refresh__.signature".to_string(),
                            ..Default::default()
                        })
                    } else {
                        None
...
impl Issue for UnsupportedServerActionIssue {
    ...
    fn title(&self) -> Vc<String> {
        Vc::cell("Server actions (\"use server\") are not yet supported in Nxpkgpack".into())
    }
