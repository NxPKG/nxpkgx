...
use nxpkg_tasks::{trace::TraceRawVcs, Upcast, ValueToString, Vc};
use nxpkg_tasks_fs::rope::Rope;
use nxpkgpack_core::{
...
impl EcmascriptChunkItemContent {
    ...
    pub async fn module_factory(self: Vc<Self>) -> Result<Vc<Code>> {
        let this = self.await?;
        let mut args = vec![
            "r: __nxpkgpack_require__",
            "f: __nxpkgpack_require_context__",
            "i: __nxpkgpack_import__",
            "s: __nxpkgpack_esm__",
            "v: __nxpkgpack_export_value__",
            "n: __nxpkgpack_export_namespace__",
            "c: __nxpkgpack_cache__",
            "l: __nxpkgpack_load__",
            "j: __nxpkgpack_dynamic__",
            "p: __nxpkgpack_resolve_absolute_path__",
            "U: __nxpkgpack_relative_url__",
            "R: __nxpkgpack_resolve_module_id_path__",
            "g: global",
            // HACK
            "__dirname",
        ];
        if this.options.async_module.is_some() {
            args.push("a: __nxpkgpack_async_module__");
        }
        if this.options.externals {
            args.push("x: __nxpkgpack_external_require__");
            args.push("y: __nxpkgpack_external_import__");
        }
        if this.options.refresh {
            args.push("k: __nxpkgpack_refresh__");
        }
        ...
        if this.options.wasm {
            args.push("w: __nxpkgpack_wasm__");
            args.push("u: __nxpkgpack_wasm_module__");
        }
        ...
        if this.options.async_module.is_some() {
            code += "__nxpkgpack_async_module__(async (__nxpkgpack_handle_async_dependencies__, \
                     __nxpkgpack_async_result__) => { try {";
        }
        ...
        if let Some(opts) = &this.options.async_module {
            write!(
                code,
                "__nxpkgpack_async_result__();\n}} catch(e) {{ __nxpkgpack_async_result__(e); }} \
                 }}, {});",
...
pub struct EcmascriptChunkItemOptions {
    ...
    /// `__nxpkgpack_refresh__` argument.
    pub refresh: bool,
    ...
    pub require: bool,
    /// Whether this chunk item's module factory should include a
    /// `__nxpkgpack_external_require__` argument.
    pub externals: bool,
    ...
    /// `__nxpkgpack_wasm__` to load WebAssembly.
    pub wasm: bool,
    pub placeholder_for_future_extensions: (),
}
