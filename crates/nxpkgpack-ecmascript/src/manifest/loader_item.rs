...
impl EcmascriptChunkItem for ManifestLoaderChunkItem {
    ...
    async fn content(self: Vc<Self>) -> Result<Vc<EcmascriptChunkItemContent>> {
        ...
        writedoc!(
            code,
            r#"
                __nxpkgpack_export_value__((__nxpkgpack_import__) => {{
                    return Promise.all({chunks_server_data}.map((chunk) => __nxpkgpack_load__(chunk))).then(() => {{
                        return __nxpkgpack_require__({item_id});
                    }}).then((chunks) => {{
                        return Promise.all(chunks.map((chunk) => __nxpkgpack_load__(chunk)));
                    }}).then(() => {{
                        return __nxpkgpack_import__({dynamic_id});
                    }});
                }});
            "#,
