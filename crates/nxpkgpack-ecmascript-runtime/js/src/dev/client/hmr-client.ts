...
import {
  addMessageListener as nxpkgSocketAddMessageListener,
  sendMessage as nxpkgSocketSendMessage,
} from "./websocket";
...
export function connect({
  ...
  addMessageListener = nxpkgSocketAddMessageListener,
  // TODO(WEB-1465) Remove this backwards compat fallback once
  // vercel/next.js#54586 is merged.
  sendMessage = nxpkgSocketSendMessage,
}: ClientOptions) {
  addMessageListener((msg) => {
    switch (msg.type) {
      case "nxpkgpack-connected":
  ...
  const queued = globalThis.NXPKGPACK_CHUNK_UPDATE_LISTENERS;
...
function subscribeToUpdates(
  ...
  sendJSON(sendMessage, {
    type: "nxpkgpack-subscribe",
    ...resource,
  });

  return () => {
    sendJSON(sendMessage, {
      type: "nxpkgpack-unsubscribe",
      ...resource,
    });
  };
}
