...
use indoc::writedoc;
use serde::Serialize;
use nxpkg_tasks::{IntoTraitRef, TryJoinIterExt, Vc};
...
impl EcmascriptDevChunkListContent {
    ...
    pub(super) async fn code(self: Vc<Self>) -> Result<Vc<Code>> {
        ...
        // When loaded, JS chunks must register themselves with the `NXPKGPACK` global
        // variable. Similarly, we register the chunk list with the
        // `NXPKGPACK_CHUNK_LISTS` global variable.
        writedoc!(
            code,
            r#"
