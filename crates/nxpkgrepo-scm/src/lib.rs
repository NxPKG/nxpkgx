...
use thiserror::Error;
use tracing::debug;
use nxpkgpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf, PathError, RelativeUnixPathBuf};
...
fn find_git_root(nxpkg_root: &AbsoluteSystemPath) -> Result<AbsoluteSystemPathBuf, Error> {
    let rev_parse = Command::new("git")
        .args(["rev-parse", "--show-cdup"])
        .current_dir(nxpkg_root)
        .output()?;
    ...
    if let Some(line) = lines.next() {
        ...
        nxpkg_root.join_unix_path(tail).map_err(|e| e.into())
    } else {
        let stderr = String::from_utf8_lossy(&rev_parse.stderr);
...
mod tests {
    ...
    fn test_symlinked_git_root() {
        ...
        let nxpkg_root = link.join_component("inside");
        let result = find_git_root(&nxpkg_root).unwrap();
