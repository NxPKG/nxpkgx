    ...
    quote! {
        ...
        impl nxpkg_tasks::debug::ValueDebug for #ident {
            ...
            async fn dbg(&self) -> anyhow::Result<nxpkg_tasks::Vc<nxpkg_tasks::debug::ValueDebugString>> {
                nxpkg_tasks::debug::ValueDebugFormat::value_debug_format(self, usize::MAX).try_to_value_debug_string().await
            }

            #[nxpkg_tasks::function]
            async fn dbg_depth(&self, depth: usize) -> anyhow::Result<nxpkg_tasks::Vc<nxpkg_tasks::debug::ValueDebugString>> {
                nxpkg_tasks::debug::ValueDebugFormat::value_debug_format(self, depth).try_to_value_debug_string().await
            }
        }
    }
    .into()
