...
pub fn value(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let cell_mode = match cell_mode {
        CellMode::New => quote! {
            nxpkg_tasks::VcCellNewMode<#ident>
        },
        CellMode::Shared => quote! {
            nxpkg_tasks::VcCellSharedMode<#ident>
        },
    };
    ...
    let (cell_prefix, cell_access_content, read) = if let Some(inner_type) = inner_type {
        (
            ...
            quote! {
                nxpkg_tasks::VcTransparentRead::<#ident, #inner_type, #ident>
            },
        )
    } else {
        (
            ...
            quote! { content },
            quote! {
                nxpkg_tasks::VcDefaultRead::<#ident>
            },
        )
    };
    ...
    let cell_struct = quote! {
        ...
        #cell_prefix fn cell(self) -> nxpkg_tasks::Vc<Self> {
            let content = self;
            nxpkg_tasks::Vc::cell_private(#cell_access_content)
        }
    };

    let into = if let IntoMode::New | IntoMode::Shared = into_mode {
        quote! {
            impl Into<nxpkg_tasks::Vc<#ident>> for #ident {
                fn into(self) -> nxpkg_tasks::Vc<#ident> {
                    self.cell()
                }
            }
        }
    ...
    let derive = match serialization_mode {
        SerializationMode::None | SerializationMode::Custom | SerializationMode::CustomForInput => {
            quote! {
                #[derive(nxpkg_tasks::trace::TraceRawVcs)]
            }
        }
        SerializationMode::Auto | SerializationMode::AutoForInput => quote! {
            #[derive(nxpkg_tasks::trace::TraceRawVcs, serde::Serialize, serde::Deserialize)]
        },
    };
    let debug_derive = if inner_type.is_some() {
        ...
        quote! {
            #[derive(nxpkg_tasks::debug::ValueDebugFormat, nxpkg_tasks::debug::internal::ValueDebug)]
        }
    };
    ...
    let new_value_type = match serialization_mode {
        SerializationMode::None => quote! {
            nxpkg_tasks::ValueType::new::<#ident>()
        },
        SerializationMode::Auto | SerializationMode::Custom => {
            quote! {
                nxpkg_tasks::ValueType::new_with_any_serialization::<#ident>()
            }
        }
        SerializationMode::AutoForInput | SerializationMode::CustomForInput => {
            quote! {
                nxpkg_tasks::ValueType::new_with_magic_serialization::<#ident>()
            }
        }
    };
    ...
    let for_input_marker = match serialization_mode {
        ...
        SerializationMode::AutoForInput | SerializationMode::CustomForInput => quote! {
            impl nxpkg_tasks::TypedForInput for #ident {}
        },
    };
    ...
    let value_debug_impl = if inner_type.is_some() {
        ...
        quote! {
            ...
            impl nxpkg_tasks::debug::ValueDebug for #ident {
...
pub fn value_type_and_register(
    ...
    quote! {
        #[doc(hidden)]
        static #value_type_init_ident: nxpkg_tasks::macro_helpers::OnceCell<
            nxpkg_tasks::ValueType,
        > = nxpkg_tasks::macro_helpers::OnceCell::new();
        #[doc(hidden)]
        pub(crate) static #value_type_ident: nxpkg_tasks::macro_helpers::Lazy<&nxpkg_tasks::ValueType> =
        ...
        #[doc(hidden)]
        static #value_type_id_ident: nxpkg_tasks::macro_helpers::Lazy<nxpkg_tasks::ValueTypeId> =
            nxpkg_tasks::macro_helpers::Lazy::new(|| {
                nxpkg_tasks::registry::get_value_type_id(*#value_type_ident)
            });
        ...
        pub(crate) fn #register_value_type_ident(
            ...
            f: impl FnOnce(&mut nxpkg_tasks::ValueType),
        ) {
            #value_type_init_ident.get_or_init(|| {
        ...
        unsafe impl #impl_generics nxpkg_tasks::VcValueType for #ty #where_clause {
            type Read = #read;
            type CellMode = #cell_mode;

            fn get_value_type_id() -> nxpkg_tasks::ValueTypeId {
                *#value_type_id_ident
            }
        }
    }
