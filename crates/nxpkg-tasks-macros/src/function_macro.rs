...
use nxpkg_tasks_macros_shared::{get_native_function_id_ident, get_native_function_ident};
...
pub fn function(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let Some(nxpkg_fn) = NxpkgFn::new(sig, DefinitionContext::NakedFn) else {
        return quote! {
    ...
    let exposed_signature = nxpkg_fn.signature();
    let exposed_block = nxpkg_fn.static_block(&native_function_id_ident);
