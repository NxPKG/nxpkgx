...
NXPKG_VERSION_FILE="${MONOREPO_ROOT_DIR}/version.txt"
# Change package.json in the example directory to point to @canary if our branch is currently at that version
NXPKG_TAG=$(cat "$NXPKG_VERSION_FILE" | sed -n '2 p')
if [ "$NXPKG_TAG" == "canary" ]; then
  jq --arg version "canary" '.devDependencies.nxpkg = $version' package.json > package.json.new
...
export NXPKG_BINARY_PATH=${MONOREPO_ROOT_DIR}/target/debug/nxpkg${EXT}

"$MONOREPO_ROOT_DIR/nxpkgrepo-tests/helpers/setup_git.sh" "${TARGET_DIR}" "false"
